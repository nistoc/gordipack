...
def cmd_status(conn, a):
    ...
    # ИНСТРУМЕНТ, в момент сдачи, где рука роли и так проходит.
    # 🪤 Почему не нормой «роль обязана назвать»: механизм, о котором надо ВСПОМНИТЬ,
    # умирает — четыре таких механизма контура имеют НОЛЬ вызовов на тысячах записей.
    # 🩸 И довод, оплаченный собой: 30.08 я прочла это предложение в 10:40 UTC и сдала
    # две работы (10:34 и 10:48), НИ В ОДНОЙ приёмщика не назвав. Не по небрежности —
