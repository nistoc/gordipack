...
def _archived_blocks_window_matches(conn, role, section, history_rows, chain_len) -> bool:
    ...
    ПОЧЕМУ НЕ РАВЕНСТВО. memory-archive.py (функция унести(), файл
    C:\\guts\\.atlas\\vnext-tools\\memory-archive.py, строки 428-450) ставит ДВЕ
    метки времени ДВУМЯ РАЗНЫМИ командами SQL внутри одной транзакции: сначала
    ...
    datetime('now'), но уже СВОИМ вызовом). SQLite держит 'now' одинаковым только
