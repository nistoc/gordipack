...
def archive_move_look(conn, role, section, body, saved, confirmed):
    ...
    ГРАНИЦЫ (по просьбе COORD, возврат карточки #605 ③) — чего эта проверка НЕ ловит:
      ...
      · ДВА ДЕЙСТВИЯ С ПАМЯТЬЮ РОЛИ В ОДНУ И ТУ ЖЕ СЕКУНДУ (настоящее сохранение
        и перенос архива) МОГУТ дать ложный 🔴 (возврат COORD, карточка #605 ③г):
        разрешение меток времени в базе — секунда, и окно проверки ③ этого не
        лечит, когда сама граница окна (saved_at S_real) той же секундой не
        отличима от чужого совпадения. Случай редкий (сохранение и перенос —
        разные ручные действия), но назван честно, а не подразумевается.
