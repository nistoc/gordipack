...
def main():
    ...
    for s in wanted:
        if s not in rows:
            ...
            # ── ЖИВЫЕ СЛОИ НЕ ЗАВИСЯТ ОТ СЛЕПКА (#129 ⑤): у новорождённого контура
            ...
            # ни строки о том, что слой не собран. Живые запросы работают без слепка —
        ...
        if s == "state":
            ...
            try:
                ...
                _c.close()
                _где = (_row[0] if _row else "") or None
            ...
            _disk = str(mezo_paths.container_root(__file__)
                        / (_где or "atlas.archs/step04_opssre/tools/disk_layer.py"))
