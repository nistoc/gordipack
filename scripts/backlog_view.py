...
уже живёт в backlog.py (`list`). Скопировав его в читалку и в сохранённую память, я получил бы ТРИ
...
отсутствие сводки не вправе отказывать сильнее, чем сама сводка (норма @TAXO 13:18 UTC).
"""
...
def reminder_lines(conn, role, top=5):
    ...
    if pools:
        ...
        if in_pool:
            out.append(f"   🎯 пул {pool_names}: твоих карточек {in_pool} — они первыми")
        else:
            out.append(f"   🎯 в пуле ({pool_names}) твоих карточек нет")
...
def reminder_block(db_path, role, top=5):
    ...
    ⚖️ Граница названа вслух: сводка задач не вправе сломать чтение ленты или
    показ сохранённой памяти. Поломка здесь печатает строку и уходит — «напоминания нет»
