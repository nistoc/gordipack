...
#    держал «v3» в день объявления отметки версии v4. Впечатанный номер протухает В ДЕНЬ
#    следующей отметки — по построению, и молча: сборщик продолжает собирать вчерашний
...
def main():
    ...
    # 1а. Режим журнала — WAL (карточка #661, 26.09). Файлы схемы v3 и новее собираются из
    # живой базы и прагму режима не несут: она была только в рукописных v1/v2, и с 08.08 каждый
    # свежий контур рождался в режиме журнала отката — читатель с открытой транзакцией держал
    # писателя до «database is locked». Режим хранится в самом файле базы, поэтому ставится один
    # раз здесь, сразу после схемы, и ответ базы ЧИТАЕТСЯ: без чтения «переключил» было бы
    # пересказом, а не фактом. Не переключилось (сетевой диск и т.п.) — сборка идёт дальше,
    # но говорит об этом вслух и попадает в отказы.
    journal = str((conn.execute("PRAGMA journal_mode=WAL").fetchone() or ["?"])[0]).lower()
    if journal == "wal":
        print("  ✅ Режим журнала: wal — читатели не держат писателя")
    else:
        soft_failures.append(f"режим журнала «{journal}», а не wal")
        print(f"  ⚠️  Режим журнала НЕ переключён: база ответила «{journal}», а не «wal» — записи "
              "будут ждать читателей; переключить: PRAGMA journal_mode=WAL")
    ...
    # 🪤 ВТОРАЯ ОШИБКА ЗДЕСЬ ЖЕ: я считал отпечаток от ТЕКСТА ФАЙЛА схемы, а проверка
    # журнала считает его от СТРУКТУРЫ БАЗЫ. Свежесобранный контур немедленно краснел
    # «схему меняли мимо журнала» — проверка была права, врал мой отпечаток.
    ...
    # обещали неправдой — код обещание не исполнял, нашёл сосед 19.08); второе делает
    # свежесобранный контур неспособным принять первое же обновление.
    # ⇒ Отпечаток снимается ЗДЕСЬ, где мы точно знаем: файлы только что положены нами
    # и никем не правлены. Позже такой уверенности нет ни у кого.
    # ⚖️ Отпечаток берётся от СОДЕРЖИМОГО, а не от байтов: у файлов, переехавших между
    # машинами, разные окончания строк, и побайтовый отпечаток объявил бы правленым всё
    ...
    # на исход запуска. Файл к тому моменту уже есть: он рождается на первом шаге.
    ...
    # прочитанного без памяти — воскресший мертвец/фантом». Проверки правы: отметка
    # прочитанного без сохранённой памяти в ЖИВОМ
    ...
    # ⇒ Кладём заготовку из templates/: роль получает launcher и §identity, проверки молчат
    ...
    # 7д. КАТАЛОГ КООРДИНАЦИИ И МОСТ СОСЕДЕЙ — КОНТУР РОЖДАЕТСЯ С МЕСТОМ ДЛЯ НИХ (#248).
    # 🪤 Третий раз тот же класс в этом файле, и лечение то же: проверка ищет каталог,
    # у новорождённого его нет — и первый экран первой роли встречает отказом, которого
    # она не создавала. Первые два раза так рождались «правил 35, а файла нет»
    # и «в phoenix нет ничего».
    # 🩸 Оплачено соседним контуром: их проверка ПЯТЬ СУТОК печатала ✅ про каталог, которого
    # у них не было вовсе. Чиним обе половины: проверка перестала выдавать пустоту за чистоту,
    # а сборка перестала рождать контур без места для координации. Одна половина без другой
