...
# NB для преемника: здесь стояло надгробие с разбором ДВУХ ошибок гарда, из-за которых
# прежняя конвертация пережила запрет («грeп с head-12» и «фильтр выбросил искомое») —
# разбор целиком живёт в шапке guard-utc.py, он и есть причина, почему гард — скрипт.
try:
    from local_time import utc_to_local
except Exception:  # noqa: BLE001 — без модуля счётчики живут: прежний показ «только UTC»
...
def _print_human(s, since_min):
    ...
    print("   (метки БД хранятся в UTC; в скобках — местное время машины запуска, карточка #384)")
