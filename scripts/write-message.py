...
def _save_phoenix_alongside(args, db_path: Path) -> list:
    ...
    try:
        ...
        if gap > 3:
            ...
            # 🩸 ДВЕ ПРАВДЫ В СОВЕТЕ — записка #5189 (PROTO) по разбору OPSSRE, записка #5188.
            # 14.09 роль OPSSRE трижды получила это напоминание и трижды отложила запись: считала,
            # что раздел упёрся в порог объёма и сначала надо резать или уносить в архив. Порог
            # запись НЕ останавливает (карточка #478) — та же ложная причина у ING 31.08 и RCC
            # (шапка save-phoenix.py). Совет, молчащий об этом, оставлял роли её ошибку.
            # Второй путь — «правок нет»: отметка взгляда без записи тела (карточка #160).
            # ⚖️ Число порога здесь НЕ пишется: оно живёт в guard-phoenix-volume.py (правило
            # «один источник — один текст»). Путь к save-phoenix.py — от своего расположения;
            # --db — только когда база не та, что рядом со скриптом: зовущий из песочницы
            # иначе отметил бы взгляд на ЖИВОЙ памяти.
            confirm = f"python {(here / 'save-phoenix.py').as_posix()} --role {args.role}"
...
def main():
    ...
    # 🪤 Даты донора («заморожен 2026-07-16», «снят 2026-08-08») стояли здесь впечатанными и
    # читались свежим контуром как ЕГО история (заявка dominal 2026-09-26 00:47 UTC). Теперь —
    # без чужих дат: где правило о заморозке md действует, его час покажет set-rule.py --show.
    parser = argparse.ArgumentParser(
        description="Записать сообщение в ленту mezosync.db — единственный источник правды; "
                    "в файлы механизм не пишет, аварийного выхода к файлам нет: база недоступна — "
                    "зови владельца живым словом и остановись, канал не изобретай.")
    ...
    parser.add_argument("--md-dir", default=None,
                        help="⛔ прежний каталог md-файлов ленты. Флаг принимается, но писать "
                             "в файлы механизм не станет: у контура-донора канал заморожен, а выход "
                             "снят (правило md-to-sqlite-phased-cutover — где оно действует). "
    ...
    parser.add_argument("--md", action="store_true",
                        help="⛔ СНЯТ (у контура-донора — словом владельца, правило "
                             "md-to-sqlite-phased-cutover). Вызов будет отклонён с объяснением: "
                             "если база недоступна — зови владельца живым словом, не пиши в файлы.")
    parser.add_argument("--no-md", action="store_true",
                        help="(устарел, no-op) В файлы механизм и так не пишет. Флаг принимается "
    ...
    # и «--role core» молча завёл бы ДЕВЯТОГО писателя. Тот же класс, что отметки прочитанного
    # в нижнем регистре (EYE #2063) — заряженное ружьё разряжаем до выстрела (П1, слово владельца 16.07 18:07 UTC).
    ...
    if msg_id is None:
        return
    if args.md:
        print("\n⛔ --md ОТКЛОНЁН: аварийного выхода к файлам у механизма нет\n"
              "   (у контура-донора снят словом владельца — правило md-to-sqlite-phased-cutover,\n"
              "   час и основание — в нём: set-rule.py --key md-to-sqlite-phased-cutover --show).\n"
              ...
              "      и остановись. У донора 2026-07-12 роль писала «куда-нибудь» и была молча отрезана.")
        return
    return
