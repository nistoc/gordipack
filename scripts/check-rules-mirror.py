...
#    контуром AIA, 13.09). Утренняя правка легла только в копию vnext-tools, а общий прогон
#    зовёт ЭТУ копию (guard-all.tool ищет рядом со скриптами первой) — перенесено сюда тем же
#    днём; приёмка bite-check-rules-mirror.py теперь испытывает обе копии.
