...
отрицала объявленную Фазу 4, а 8 призрачных отметок прочитанного в нижнем регистре прожили 5 суток: расхождение
...
ЧТО ПРОВЕРЯЕТ (каждая проверка — оплаченный урок 16.07):
  ...
  ③ хронология id        — «id растёт ⇒ время растёт», иначе отметка прочитанного прыгает (ретро-импорт)
...
# 🪤 Найдено ЗАПУСКОМ проверок на свежесобранном контуре: с впечатанными путями они
...
RESULTS = []
# ⚡ КРАТКИЙ ВЫВОД ПО УМОЛЧАНИЮ (карточка #593). FULL включается флагом --full в main().
...
def tool(name):
    ...
    (нашего). На свежесобранном контуре проверки честно краснели «гард не найден» —
...
def sub_guard(name, script, skip, *extra, script_path=None):
    ...
    if r.returncode == 0:
        # ⚖️ Строки-проброс считаем ДО check(): если они есть, «✅ имя» обязано печататься
        # СРАЗУ (force_print) — предупреждению нужна подпись рядом, иначе оно повиснет без
        # привязки в кратком выводе (карточка #593). Нечего пробрасывать — имя копится в
        # ЗЕЛЁНЫЕ молча, как любая другая тихая проверка.
        forward = [l for l in lines if l.lstrip().startswith(("⚠️", "ℹ️"))]
        check(name, True, lines[-1] if lines else "", force_print=bool(forward))
...
def main():
    ...
    ap.add_argument("--full", action="store_true",
                    help="печатать каждую зелёную проверку и каждый отвеченный вопрос соседа отдельной строкой")
    # --db заведён 08.08 14:39 UTC, карточка #50. ЗАЧЕМ: гард ходил ТОЛЬКО в живую базу ⇒
    # проверить его самого было нечем — подложить образец сохранённой памяти можно было лишь испортив
    # живую. Проверка, которую нельзя запустить по копии, нельзя и удостоверить: её «зелёное»
    # приходится принимать на веру, а это ровно то, чего проверки и должны избавлять.
    ...
    # запускает эту проверку НА КОПИИ — и получала красное из живого контура.
    ...
    #    помнил его позвать. @TAXO 05.08 закрыла класс у себя ДИСЦИПЛИНОЙ (вписала шаг в сохранённую память),
    ...
    sub_guard("метка-под-UTC", None, skip, "--since-hours", "24", "--db", str(DB),
    ...
    sub_guard("самопроверка метки-под-UTC", None, skip, "--selftest", "--db", str(DB),
              script_path=tool("guard-phoenix-time.py"))
    ...
    sub_guard("память: правка мимо инструмента", None, skip,
              "--db", str(DB), script_path=tool("check-phoenix-invariant.py"))
    ...
    sub_guard("сроки годности правил", None, skip,
              "--db", str(DB), script_path=tool("guard-rule-expiry.py"))
    ...
    sub_guard("отставание разделов памяти", None, skip,
              "--db", str(DB), script_path=tool("guard-section-lag.py"))
    ...
    sub_guard("объём разделов памяти", None, skip,
              "--db", str(DB), script_path=tool("guard-phoenix-volume.py"))
    # ⑨-в ПРАВО В ПАМЯТИ БЕЗ ЗАПИСИ РЕЕСТРА — включён 2026-09-05 (карточка #546, заявка
    # @COORD по слову владельца 05:00:53; правило right-to-registry-same-move).
    # 🎯 ЧТО ЛОВИТ: строку таблицы §ПРАВА, которая объявляет право, но не ссылается ни на
    #    запись реестра, ни на честную пометку «⛔ НЕ В РЕЕСТРЕ — след: …».
    # 🩸 ЖИВОЙ СЛУЧАЙ (карточка #543): право STUD на подъём стенда жило в памяти ПЯТЬ СУТОК,
    #    а в реестре его не было. Стартовая сводка печатала «прав именных нет», память —
    #    «есть»; обе честны, сравнивал никто ⇒ роль пошла спрашивать владельца заново,
    #    и он ответил дважды за минуту двум ролям об одном.
    # 📏 Замер перед включением (05.09 06:28 UTC): 9 ролей с таблицей прав, 17 строк-прав,
    #    🔴 14 ⇒ включается КРАСНЫМ НАМЕРЕННО — как отставание разделов 28.08. Долг видим
    #    поимённо, руки у каждой роли свои: чужую память проверка не правит и не читает
    #    ничего, кроме памяти и реестра.
    # Приёмка: bite-rights-registry.py — 21 случай, 4 порчи, каждая краснит СВОЙ случай.
    sub_guard("права: строка памяти без записи реестра", None, skip,
              "--db", str(DB), script_path=tool("guard-rights-registry.py"))
    # ⑨-г ССЫЛКА НА РАЗДЕЛ ПАМЯТИ БЕЗ АДРЕСАТА — включена 2026-09-15 (карточка #643 ⑥: приём проверки ⑨
    #    из wake-check.py контура AIA, слово владельца 2026-09-15 14:05:59 UTC «перенести приёмы»).
    # ⚖️ МЯГКО (--soft: код 0, находки строками «⚠️»). Замер перед включением на копии живой памяти:
    #    243 ссылки найдены, без адресата 1 — и та ложная (слово «раздел» о выводе другого инструмента).
    #    Провал общего прогона по сигналу с известной долей ложных находок обвинял бы чужую роль за
    #    пробел словаря самой проверки; перевод в провал — отдельным решением после замера качества.
    sub_guard("ссылка на раздел памяти без адресата", None, skip,
              "--db", str(DB), "--soft", script_path=tool("guard-memory-section-refs.py"))
    # ⑨-д session_id РОЛИ ЖИВ ЛИ ОН В ХРАНИЛИЩЕ СЕССИЙ ПРИЛОЖЕНИЯ — включён 2026-09-24
    #    (карточка #649, зона @PROTO). МЯГКО, тем же приёмом, что и соседняя проверка выше:
    #    session_id переживает возобновление чата, но сама сессия может уйти в архив или
    #    пропасть из хранилища — реестр продолжает выглядеть годным молча.
    sub_guard("session_id ролей жив в хранилище сессий приложения", None, skip,
              "--db", str(DB), "--soft", script_path=tool("guard-role-sessions-live.py"))
    ...
    # 🔴 Карточка #555, находка OPSSRE 05.09 (записка #4839 §③): шаг «навыки против свода» звался
    #    БЕЗ --db — прогон, натравленный на копию, читал ЖИВУЮ базу и зеленел там, где копия красна.
    #    Класс уже был оплачен строкой ниже про check-retired-mechanism и ВЕРНУЛСЯ на новом шаге:
    #    починка жила в каждом вызове поодиночке. Теперь --db передаётся ВСЕМ шагам, которые его
    #    принимают (восемь через tool()); guard-stub-expectations --db не знает — отдельная заявка.
    sub_guard("навыки против свода", None, skip,
              "--db", str(DB), script_path=tool("guard-skills-fresh.py"))
    # ⑮ ЗАМЕР ПАМЯТИ — кандидаты на устаревание по всем живым ролям. Включён 2026-08-13
    # (карточка #185): утренний замер дал 49 кандидатов, из них живых лжей — 5; после
    # починки проверки и пяти пометок о снятии стало 0. Порог стабилизации назван владельцу:
    # МЕНЬШЕ 10 суммарно и неделя без вмешательства. Порог без проверки — обещание,
    # проверка вне общего прогона — памятка (формула оплачена 06.08).
        ...
        if not sieve_tool.exists():
            ...
            for role_ in alive:
                ...
                if m:
                    ...
                    per.append(f"{role_} ⁇")   # проверка не запустилась — видно, а не ноль
            if alive:
    ...
    # Разовая чистка без проверки отрастает за месяц: сосуд наполняется обратно, и никто
    # не замечает. ИНФОРМАЦИОННЫЙ: объём сам по себе не грех (роль честно наживает уроки),
    # красить нельзя — видимой должна быть ДЕЛЬТА от точки отсчёта. Точка отсчёта — В БАЗЕ (meta),
    # не в коде: впечатанное в проверку число — тот же класс, который она и ловит.
    # Потерянная точка отсчёта говорит о себе вслух, а не молчит и не выдумывает ноль.
    if "память" not in skip:
    ...
    # («в сохранённой памяти 4, в базе 7» — цитата обновлена 2026-09-06 ОДНИМ ходом
    # с переводом самого вывода: карточка #577 ④, порознь нельзя). Предмет — числа, которые база отвечает сама: карточки, правила,
    # шаги схемы; число ролей красное даже ВЕРНЫМ — канон запрещает держать его текстом.
    #
    # ⚖️ ЗОВЁТСЯ ЖЁЛТЫМ, А НЕ КРАСНЫМ, И ЭТО РЕШЕНИЕ, А НЕ ПОЛУМЕРА. Прогон до применения дал
    # две находки, обе НАСТОЯЩИЕ и обе в ЧУЖОЙ сохранённой памяти (@ING: «ролей 8» при девяти) — то есть
    # красное, которое я не могу погасить своей рукой: память чужая, роль правит её сама.
    # 🎯 Мой же довод из проверки копий: красное, которое некому погасить, приучает пролистывать
    ...
    # при пробуждении) породили 24 копии относительной формы в сохранённых памятях. Относительный вызов при
    ...
    # ЖЁЛТЫМ — сохранённая память ролей: её чинит каждая роль сама, вечно-красный гард обесценил бы сигнал (урок ⑪).
    ...
    # либо переставала верить гарду. Второе дороже: проверка, которой не верят, не защищает ничего.
    ...
    # одновременно и порознь: я — веткой здесь, он — отдельным признаком. Две проверки одного
    # класса опаснее одной отсутствующей: расходиться они начинают МОЛЧА, и роль верит тому,
    ...
    # ⚠️ БАЗУ И КОРЕНЬ ПЕРЕДАЁМ ЯВНО. Первая редакция применения звала признак без аргументов —
    ...
    # отвечает другое, и оба ответа выглядят одинаково уверенно. Поймано приёмкой применения.
    ...
    # 🪤 Строка пробуждения CORE приказывала звать полный прогон проверок по пути, где файла нет (он в .run/).
    # Отказ был громкий — код 127, — но стоял ровно в разделе про третий исход «НЕ ЗАПУСТИЛАСЬ»:
    # тот же вызов через конвейер (полный прогон печатает сотни строк) дал бы пустоту вместо красного.
    ...
    # Форма и существование — РАЗНЫЕ свойства; второе до 10.08 не покрывала ни одна проверка.
    ...
    # ⑥-Д НЕВИДИМЫЕ УПРАВЛЯЮЩИЕ БАЙТЫ В ФАЙЛАХ ТУЛКИТА — включён 2026-09-15 (карточка #643 ⑤,
    #    слово владельца 2026-09-15 14:05:59 UTC «бери guard-control-chars.py в пакет»).
    # 🎯 Инструмент передан контуром AIA (их находка STUD-A, записка #5223, 2026-09-10):
    #    обратный слэш в передаваемом коде уполовинивается слоем вызова, и одиночный байт
    #    ниже 0x20 в тексте невидим глазу, а исполнение ломает. Разбор перед взятием — там же
    #    (карточка #643): инструмент общий, привязки к AIA в нём нет.
    # ⚠️ БЕЗ АРГУМЕНТОВ — инструмент сам вычисляет свои каталоги по умолчанию от собственного
    #    расположения (обе раскладки: контейнер и пакет) и только ЧИТАЕТ: --fix/--unescape
    #    из общего прогона не зовутся никогда — лечение только по явному ручному вызову.
    # 📏 Замер перед включением (15.09, копия дерева): 351 файл, найдено 0 — включается зелёным.
    #    Инструмент в зоне @PROTO (vnext-tools), зовём по пути, как соседние проверки.
    sub_guard("невидимые управляющие байты", None, skip,
              script_path=tool("guard-control-chars.py"))
    ...
    # поверят. Это же блокировало полный прогон проверок у CORE. ⇒ критерий — РАСХОЖДЕНИЕ С КОММИТОМ (git status
    ...
    #
    # ЗАЧЕМ (17.07 13:47 UTC): сохранённая память — ЕДИНСТВЕННОЕ, что переживает чат. Лента расскажет
    # новорождённому, что роль СКАЗАЛА, но не то, что у неё в рабочем дереве, в долгах и в
    # голове. Замер: STUD отставал на 18.8 ч, ING на 16.8 ч — сохранённая память STUD обещала «Открытых
    ...
    # now(): дормантная роль (RCC) молчит по слову владельца — её сохранённая память совпала с её нотой и
    ...
    # неверным — и потому ничего не сообщает»: свежая сохранённая память при потоке нот ПОСЛЕ неё всё
    # равно протух СОДЕРЖАНИЕМ — активная роль наговаривает на новую сохранённую память и за полчаса.
    # Жёлтый, не красный: exit-код не трогаем; порог 5 — первая приёмка, подстраивать по ложным.
    NOTES_AFTER_SNAP = 5
    ...
    # обе невидимы. Величина называлась «свежесть сохранённой памяти», а мерила свежесть САМОЙ СВЕЖЕЙ
    ...
    #    поэтому дат теперь две, и проверка обязана спрашивать ту, которая про взгляд.
    ...
    for role, snap in conn.execute(
        ...
        if lag_h > STALE_HOURS:
            stale.append(f"{role} +{lag_h:.1f} ч (память сохранена {snap}, записка {note})")
            continue
        n_after = conn.execute(
            "SELECT COUNT(*) FROM messages WHERE writer_role = ? AND timestamp > ?",
            (role, snap)).fetchone()[0]
        if n_after >= NOTES_AFTER_SNAP:
            drifted.append(f"{role} — {n_after} записок после сохранения памяти {snap}")
    # ⑪ ЖЁЛТЫЙ, не красный (рекомендация DELTA §2, замер PROTO #2676②): активная роль наводит
    # на сохранённую память ноты быстрее, чем она сохраняется, ⇒ ⑪ горел у 4 ролей из 8 ПРИ КАЖДОМ прогоне и
    ...
    # 🪤 ПЯТАЯ ПРИЁМКА, и она сняла `--diff-filter=A` (замер @opssre 07.08 17:00 UTC, карточка #120).
    ...
    # было «когда появилось ЭТО СОДЕРЖИМОЕ». Утром так же промахивалась проверка строки
    ...
    sub_guard("зеркало правил", None, skip,
              "--db", str(DB), script_path=tool("check-rules-mirror.py"))
    ...
    # Воспроизведено: оно встало под «сохранённая память: содержание отстаёт» и читалось как «5 старых
    # СОХРАНЁННЫХ ПАМЯТЕЙ» — бессмыслица, но правдоподобная. Предмет строки задавался тем, что случайно
    ...
    # ⚖️ ЧИСЛО НАШИХ ПИСЕМ ПЕЧАТАЕТСЯ ВСЕГДА, даже когда исключать было НЕЧЕГО (карточка #573):
    # молча пропущенное читается как «проверено», а проверено оно не было — оно ИСКЛЮЧЕНО.
    # Строка говорит, СКОЛЬКО файлов вышло из-под суда и по какому признаку.
    # 🩸 ПЕРВАЯ РЕДАКЦИЯ ПЕЧАТАЛА ЕЁ ТОЛЬКО ПРИ НЕНУЛЕ — и пояснение было ШИРЕ кода:
    # обещало «всегда», а при нуле молчало. Нашёл @COORD приёмкой на трёх стендах
    # (наших 1/чужих 1 · наших 1/чужих 0 · наших 0/чужих 1) — молчал ровно третий.
