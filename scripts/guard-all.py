...
# ⚡ ПУТИ ВЫВОДЯТСЯ ОТ РАСПОЛОЖЕНИЯ ЭТОГО ФАЙЛА, А НЕ ВПЕЧАТАНЫ (#145, 10.08 01:15 UTC).
# 🪤 Здесь стояло: SCRIPTS = «<КОНТУР>/.mezosync/scripts» (заглушка, которой нет никогда)
# и DB = «C:\guts\.atlas\.mezosync\mezosync.db» — АБСОЛЮТНЫЙ ПУТЬ ЧУЖОЙ МАШИНЫ.
# Найдено ЗАПУСКОМ сторожей на свежесобранном контуре: они пожаловались на слепки НАШИХ
# ролей, стоя в чужом контуре, — то есть читали базу разработчика шаблона.
# ⇒ У потребителя это либо падение, либо (страшнее) зелёное про чужие данные.
# Класс тот же, что лечили внутри своего контура (#153); в шаблон починка не доехала:
# **починка, не доехавшая до образца, лечит одного и учит неправде всех.**
import mezo_paths  # noqa: E402

SCRIPTS = Path(__file__).resolve().parent
DB = mezo_paths.default_db(__file__)
COORDINATION = mezo_paths.container_root(__file__) / "coordination"
...
def tool(name):
    """Путь к звену-инструменту в ЛЮБОЙ раскладке (#145, 10.08 01:18 UTC).

    🪤 Здесь стояло жёсткое `SCRIPTS.parent.parent / "vnext-tools" / name` — раскладка
    ОДНОГО контура (нашего). На свежесобранном контуре сторожа честно краснели «гард не
    найден: .../fresh145g/vnext-tools/...» — каталога с таким именем у потребителя нет
    и не будет. Отказ был громким, и это спасло; но чинится он не заглушкой, а поиском.
    ⇒ Ищем по известным раскладкам ПО ОЧЕРЕДИ; не нашли — отдаём первый кандидат, чтобы
    сообщение об отсутствии называло ОЖИДАЕМОЕ место, а не пустоту.
    """
    candidates = [
...
def main():
    ...
    container = mezo_paths.container_root(__file__)   # #145: не путь чужой машины
