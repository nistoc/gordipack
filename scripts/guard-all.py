...
SCRIPTS = Path(r"<КОНТУР>/.mezosync/scripts")
DB = Path(r"C:\guts\.atlas\.mezosync\mezosync.db")
COORDINATION = Path(r"C:\guts\.atlas\atlas.archs\.mezosync\coordination")
...
def main():
    ...
    sub_guard("метка-под-UTC", None, skip, "--since-hours", "24",
              script_path=SCRIPTS.parent.parent / "vnext-tools" / "guard-phoenix-time.py")
    ...
    rule_body = conn.execute(
        "SELECT body FROM rules WHERE rule_key='md-to-sqlite-phased-cutover'").fetchone()[0]
    ...
    sub_guard("зеркало правил", None, skip,
              script_path=SCRIPTS.parent.parent / "vnext-tools" / "check-rules-mirror.py")
    ...
    if fresh or old:
        head = (f"⚠️ мост: СВЕЖИХ записок БЕЗ РАЗБОРА — {len(fresh)}: " + " · ".join(fresh[:4])
                ...
                + "\n   — отправка в согласованное место НЕ ДОСТАВКА, пока у места нет наблюдателя"
                  ...
                  "\n      python C:\\guts\\.atlas\\.mezosync\\scripts\\write-message.py"
