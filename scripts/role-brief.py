...
def build(conn, role, полный=False, full=False, db_path=None):
    ...
    def формы(out):
        ...
        out.append(("_HINT_", "role-brief-порядок-пробуждения",
                    "   ⚖️ порядок пробуждения: гарды → наказ → память → лента → ПЕРВЫЙ "
    ...
    def освободилось(out):
        # ═══ Карточка #488 (слово владельца 30.08 11:55 UTC «1 + 2»): ВЗЯТИЕ инструмента
        # расходится само — кто упёрся, тот увидел. СНЯТИЕ не расходится никак.
        # 📏 Замер 30.08 11:36 UTC: объявлений 127, снято раньше объявленного срока 126
        # (99%), медиана зазора 36.7 мин, сумма «числится занятым, а свободен» 83.5 часа.
        # 🔴 И почему словами этого мало: строка «не жди срока, спроси состояние» стои́т
        # в тексте отказа с 29.08 (по замеру @OPSSRE). 30.08 её прочли @STUD (11:10)
        # и @CHROME (11:23) — и ОБА написали в ленте, что дождутся срока 12:36, тогда
        # как объявление сняли в 11:28:56. Предупреждение требует лишнего действия —
        # пойти и спросить; здесь роль не делает НИЧЕГО.
        # ⚖️ Окно показа — 12 часов от снятия, и оно названо словом: раздел без окна
        # стал бы вечным, а вечную строку перестают читать. Отметки «показано» нет
        # намеренно: стартовая сводка ЧИТАЕТ, и писать из неё значило бы завести
        # читающему инструменту скрытое состояние.
        try:
            rows = conn.execute(
                "SELECT w.tool, w.refused_at, l.role, l.released_at, COALESCE(l.note,'') "
                "FROM tool_lease_waits w JOIN tool_leases l ON l.id = w.lease_id "
                "WHERE UPPER(COALESCE(w.role,'')) = ? "
                "  AND l.released_at IS NOT NULL "
                "  AND l.released_at > datetime('now', '-12 hours') "
                "ORDER BY l.released_at DESC", (role.upper(),)).fetchall()
        except sqlite3.Error:
            # Копия базы старше шага — это НЕ «никто не ждал»: сказать нечего, и молчим,
            # а не печатаем ноль. Ложный ноль хуже пустоты.
            return
        if not rows:
            return                       # ноль — обычный день, вечная строка была бы шумом
        out.append(f"🔓 ОСВОБОДИЛОСЬ, ПОКА ТЫ ЖДАЛА: {len(rows)} "
                   f"(показываю 12 ч от снятия)")
        for инстр, отказ, кто, снято, итог in rows[:5]:
            out.append(f"   {инстр} — ждала с {отказ} UTC, {кто} снял(а) в {снято} UTC")
            if итог:
                out.append(f"      что изменилось: {итог[:90]}")
        if len(rows) > 5:
            out.append(f"   … и ещё {len(rows) - 5}")
    section("освободилось", освободилось, out)
    ...
    def свод(out):
        ...
        out.append(f"📜 СВОД: активных правил {n} —")
        out.append(("_HINT_", "role-brief-свод-целиком",
                    "   в стартовую сводку не режется (граница в шапке); целиком: python {S}/set-rule.py --list".format(S=S),
                    "   в стартовую сводку не режется (граница в шапке); целиком: python {S}/set-rule.py --list".format(S=S)))
    ...
    def ответы(out):
        ...
        if not st or st[0] != "active":
            ...
            return
        out.append(("_HINT_", "role-brief-ответы-владельцу",
                    "🗣 ОТВЕТЫ ВЛАДЕЛЬЦУ: перед КАЖДЫМ ответом владельцу перечитай правило "
...
def main():
    ...
    role = a.role.upper()
    try:
        # readonly=True — карточка #391: характер вызова называет САМ инструмент, а не
        # эвристика по имени файла. Стартовая сводка ТОЛЬКО читает (соединение ниже —
        # mode=ro), но имя «role-brief.py» под читающий образец не подходит, и объявление
        # о правке судило её пишущей.
        # 🩸 ОПЛАЧЕНО 30.08 12:00–12:06 UTC, моей же рукой: объявив правку сводки,
        # я отказала в стартовой сводке ВСЕМ ДЕВЯТИ ролям разом — включая себя. Девять
        # отказов за четыре секунды легли в новый след ожиданий и этим себя и показали.
        # ⚡ Цена ошибки тут наибольшая в контуре: сводку читает каждая роль каждую сверку,
        # и отказ в ней — это отказ в ЕДИНСТВЕННОМ месте, где роль узнаёт своё положение.
        db = Path(resolve_db(a.db, __file__, readonly=True))
    ...
    for item in build(conn, role, полный=a.waiting, full=a.full, db_path=db.as_posix()):
        if isinstance(item, tuple) and len(item) == 4 and item[0] == '_HINT_':
            _, key, text, text_for_hint = item
            mezo_hints.подсказка(conn, mezo_hints.кто_читает(a.actor, role), key, text_for_hint,
                                  full=a.full, db_path=db.as_posix())
        else:
            print(item)
    conn.close()
