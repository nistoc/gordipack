...
def _who_is_calling() -> str | None:
    """Имя роли, которой сейчас отказали. Возвращает None, если узнать нечем.

    ⚖️ ПОЧЕМУ ДВА ИСТОЧНИКА, А НЕ ОДИН. MEZO_ROLE ставит только та роль, которая правит
    СВОЁ объявление, — упирающийся в ЧУЖОЕ его почти никогда не ставит (замер 30.08:
    оба живых случая, @STUD и @CHROME, звали инструмент без него). Зато имя роли стои́т
    в её же командной строке: у карточек это --actor, у объявлений и ленты --role.
    Отказ происходит ВНУТРИ процесса зовущего ⇒ sys.argv — это его собственный вызов,
    а не догадка о нём.
    ⛔ И честная граница: узнаётся не всегда. Вызов без имени роли в доводах остаётся
    безымянным, и таких мы считаем ОТДЕЛЬНО — выдуманное имя было бы хуже пустого.
    """
...
def _remember_wait(db, lease_id: int, tool: str) -> None:
    """Отказ САМ записывает ожидание: роль уже позвала инструмент, и это её заявка.

    ⚖️ Почему это лежит здесь, а не в отдельной команде «встань в очередь»: механизм,
    о котором надо ВСПОМНИТЬ, умирает — четыре таких в контуре имеют НОЛЬ вызовов
    на тысячах записей. Здесь роль не делает НИЧЕГО лишнего.
...
def check(db, script_file, quiet: bool = False, readonly: bool | None = None) -> None:
    ...
    # Ожидание записывается ДО печати отказа: если запись почему-то не удастся, роль
    # всё равно получит свой отказ — он важнее следа о нём.
    _remember_wait(db, _id, tool)
    print(head + "\n   ⛔ Инструмент ПИШУЩИЙ — отказ: правка во время чужой отладки "
                 ...
                 "запиской.\n"
                 "   🫱 Ждать НЕ надо: твоё ожидание записано. Роль, снимающая объявление, "
                 "увидит твоё имя и час,\n"
                 "     а освободившееся появится в твоей стартовой сводке (role-brief) "
                 "само (карточка #488).", file=sys.stderr)
    sys.exit(3)
...
def _cli() -> int:
    ...
    con.commit()
    # ═══ Карточка #488: СНЯТИЕ НАЗЫВАЕТ ЖДУЩИХ. Взятие расходится само — кто упёрся,
    ...
    if ждущие:
        ...
        print("   👉 СКАЖИ ИМ СТРОКОЙ В ЛЕНТЕ: снятие само до них не доходит. "
              "Своё они увидят в стартовой сводке,\n"
              "     но живое слово быстрее — 30.08 двое ждали срока час после снятия")
    return 0


def _waiters(con, lease_id: int):
    """Кто упёрся в этот инструмент, пока объявление было живым.

    Возвращает (роль, инструмент, час отказа, часов назад). Роль бывает None —
    см. _who_is_calling. Молчит, если таблицы нет: копия базы может быть старше шага,
    и снятие объявления не имеет права от этого падать.
    """
    try:
        return con.execute(
            "SELECT role, tool, refused_at, "
            "       CAST((julianday('now') - julianday(refused_at)) * 24 AS INTEGER) "
            "FROM tool_lease_waits WHERE lease_id = ? ORDER BY refused_at",
            (lease_id,)).fetchall()
    except sqlite3.Error:
        return []
