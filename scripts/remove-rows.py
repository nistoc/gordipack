...
было словами, а не проверкой — «--where 1=1 --apply» сняло бы всю таблицу. Теперь:
  ...
  ⚪ ГРАНИЦА (OPSSRE, записка #5273): значение БЕЗ цифр и кавычек проходит — «rowid = TRUE»,
    «rowid = length(typeof(rowid))». Образцом по тексту это не закрыть: такая запись по форме
    не отличается от имени колонки или функции. Держат показ по умолчанию и след в audit_log.
...
# Значение ТЕКСТОМ в условии: строка в кавычках или число в ЛЮБОЙ записи — «5», «3.14»,
# «0x5», «5e0» (граница OPSSRE Н3: прежний образец \b\d+(?:\.\d+)?\b пропускал две последние,
# и строка снималась). Имя колонки с цифрой внутри («col2») не задевает: граница слова между
# буквой и цифрой не проходит.
LITERAL_IN_WHERE = re.compile(r"""['"]|\b\d[\w.]*""")
...
def where_refusal(where: str, params: list):
    ...
    if LITERAL_IN_WHERE.search(where):
        # граница OPSSRE Н5: имя колонки в двойных кавычках ловится как «строка» — отказ
        # остаётся (кавычки в условии — тот же путь для значения), но с подсказкой
        hint = " · имя колонки пиши без кавычек" if '"' in where else ""
        return ("в --where значение текстом (число или строка в кавычках) — значения только"
                " через плейсхолдер ? и --param" + hint)
...
def main():
    ...
    ap.add_argument("--where", required=True,
                    help="условие SQL БЕЗ слова WHERE, значения — плейсхолдерами ? и --param, "
                         "не текстом внутри условия. Число и строку в кавычках инструмент "
                         "отвергает сам; значение без цифр и кавычек (TRUE, выражение) "
                         "проходит — сверь показ, прежде чем добавить --apply")
