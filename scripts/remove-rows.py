...
def preview(table: str, where: str, params: tuple, basis: str, old_rows: list) -> None:
...
def main():
    ...
    if not write_mode:
        ...
        conn.close()
        preview(args.table, args.where, tuple(args.param), args.basis, old_rows)
        return 0

    try:
        removed, diff_md = remove_rows(conn, args.actor, args.table, args.where,
                                       tuple(args.param), args.basis)
    except sqlite3.Error as exc:
