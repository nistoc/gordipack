...
# ⚡ ПУТИ ВЫВОДЯТСЯ (#145, 10.08 06:49 UTC). 🪤 Здесь стояла заглушка «<КОНТУР>» —
# её СОЗДАЛ переносчик, «обезличив» исполняемую строку живой копии: у потребителя сторож
# падал «рабочего каталога нет: <КОНТУР>…» на первом же прогоне свежесобранного контура.
# Обезличивание примера — норма; обезличивание ИСПОЛНЯЕМОЙ строки — изготовление поломки.
RUNTIME = Path(__file__).resolve().parent
# Зеркало-репозиторий у нового контура НЕ СУЩЕСТВУЕТ, и это не дефект, а состояние
# «бэкап ещё не заведён»: путь ниже — наш (контейнер автора шаблона); отсутствие зеркала
# сторож обязан говорить СЛОВАМИ «заведи бэкап», а не падать заглушкой.
...
def main():
    ...
    # ⚡ ЗЕРКАЛА-РЕПОЗИТОРИЯ МОЖЕТ НЕ БЫТЬ ВОВСЕ — И ЭТО СОСТОЯНИЕ, А НЕ ПОЛОМКА (#145).
    # 🪤 Прежний код ТИХО СОЗДАВАЛ пустой каталог (mkdir) и дальше кричал «⛔ НЕТ В РЕПО (46)»
    # — новорождённый контур встречал первого читателя красным про бэкап, которого он и не
    # заводил, да ещё и с пустым каталогом-обманкой на диске. Красное на пустом месте учит
    # пролистывать красное; каталог-обманка учит «бэкап есть».
    # ⇒ Нет каталога = сказать СЛОВАМИ, что бэкап не заведён, и как завести. Не красное.
    if not REPO.is_dir():
