...
def main():
    ...
    for role, cursor, read_at in conn.execute(
        ...
        if last_note is None or (now - last_note).total_seconds() > ACTIVE_MIN * 60:
            ...
            if долг:
                вне_окна.append({"role": role, "lag_notes": долг})
            continue
        active.append(role)
    ...
    вне_окна.sort(key=lambda x: -x["lag_notes"])
    долг_вне = sum(x["lag_notes"] for x in вне_окна)
    всего_ролей = len(active) + len(вне_окна)
    ...
    if args.json:
        ...
        print(json.dumps({"offenders": offenders, "active": active,
                          "out_of_window": вне_окна, "out_of_window_debt": долг_вне},
                         ensure_ascii=False))
        sys.exit(1 if offenders else 0)

    def сказать_про_вне_окна():
        ...
        if not вне_окна:
            return
        имена = " · ".join(f"{x['role']} {x['lag_notes']}" for x in вне_окна)
        print(f"   ⚠️ вне окна ({ACTIVE_MIN} мин) {len(вне_окна)} ролей, их долг "
              f"НЕ ПРОВЕРЯЛСЯ: {долг_вне} непрочитанных ({имена})")
    ...
    if not active:
        print(f"⚠️ активных ролей НЕТ (из {всего_ролей}) — проверять было НЕКОГО, "
              "и это не «долгов нет»")
    else:
        print(f"✅ активных ролей {len(active)} из {всего_ролей} — читают все")
    сказать_про_вне_окна()
    sys.exit(0)
