...
чужие, потому что таймер снят прошлой сессией, а сохранённая память не заставила его поднять. Дисциплина
...
читать: её отметка прочитанного не должна отставать от конца ленты больше чем на LAG_NOTES нот, а время последнего
...
ГРАНИЦА ЧЕСТНОСТИ: гард видит ОТМЕТКУ ПРОЧИТАННОГО, а не понимание. Роль может подтвердить прочтение и не
...
LAG_NOTES = 8       # допустимое отставание отметки прочитанного от конца ленты
...
def main():
    ...
    offenders, active, out_of_window = [], [], []
    for role, cursor, read_at in conn.execute(
        ...
        if last_note is None or (now - last_note).total_seconds() > ACTIVE_MIN * 60:
            # ⚡ СПЯЩАЯ РОЛЬ НЕ СУДИТСЯ — НО И НЕ ИСЧЕЗАЕТ ИЗ ОТЧЁТА (заявка @COORD, записка
            # #3767). Прежняя редакция делала здесь `continue`, и роль пропадала бесследно;
            # итог печатал «все читают ленту» при 1280 непрочитанных у восьми ролей из девяти.
            # 🩸 Замер, который это вскрыл: в 17:24 признак был КРАСНЫМ (назвал роль честно),
            # а в 19:13 стал зелёным — не потому что кто-то прочёл, а потому что та роль
            # замолчала на 166 минут и вышла из окна. Долг вырос с 93 до 98 и стал невидим.
            # ⚖️ Окно оставлено намеренно: краснеть на спящих — вернуть вечно-красный, который
            # учит не верить красному. Лечится не отменой окна, а тем, что зелёное перестаёт
            # означать больше, чем проверено: «активные читают» ≠ «с чтением в контуре порядок».
            # 🎯 Это та же формула, что в проверке замороженных файлов: ПУСТОЙ НАБОР НЕ ДОЛЖЕН
            # ПРОХОДИТЬ ЗЕЛЁНЫМ. Здесь зелёное означало «проверять было некого».
            lag = conn.execute(
                "SELECT COUNT(*) FROM messages WHERE id > ? AND writer_role <> ?",
                (cursor or 0, role)).fetchone()[0]
            if lag:
                out_of_window.append({"role": role, "lag_notes": lag})
            continue
        active.append(role)
    ...
    out_of_window.sort(key=lambda x: -x["lag_notes"])
    out_of_window_lag = sum(x["lag_notes"] for x in out_of_window)
    total_roles = len(active) + len(out_of_window)

    if args.json:
        # ⚠️ Вне окна попадает и в машинный ответ: встраивающий не должен узнавать
        #    о непроверенных ролях иначе, чем зовущий глазами.
        print(json.dumps({"offenders": offenders, "active": active,
                          "out_of_window": out_of_window, "out_of_window_debt": out_of_window_lag},
                         ensure_ascii=False))
        sys.exit(1 if offenders else 0)

    def report_out_of_window():
        """Долг НЕ проверенных ролей называется ВСЕГДА — и при красном, и при зелёном.

        ⛔ Не делает прогон красным: спящая роль не обязана читать, и требовать этого
        от закрытого чата нельзя. Строка нужна ровно для одного — чтобы «активные читают»
        не прочиталось как «с чтением в контуре порядок».
        """
        if not out_of_window:
            return
        role_list = " · ".join(f"{x['role']} {x['lag_notes']}" for x in out_of_window)
        print(f"   ⚠️ вне окна ({ACTIVE_MIN} мин) {len(out_of_window)} ролей, их долг "
              f"НЕ ПРОВЕРЯЛСЯ: {out_of_window_lag} непрочитанных ({role_list})")
        print("      Это не обвинение: спящая роль читать не обязана. Но и не «порядок» — "
              "просто не проверено")
    ...
    if offenders:
        ...
        print("   Подпись невзведённого таймера сверки. Роль пишет свежее, чем читает —")
        print("   значит зовущие её ноты не дойдут, и пинг придёт от владельца, а не от механизма.")
        report_out_of_window()
        sys.exit(1)

    # ⚡ ФОРМУЛИРОВКА ИТОГА: «читают все АКТИВНЫЕ», а не «все читают ленту». Прежняя строка
    # была верна формально и лгала по существу — она отвечала на вопрос «есть ли долг
    # у писавших за 90 минут», а звучала как ответ про весь контур.
    if not active:
        print(f"⚠️ активных ролей НЕТ (из {total_roles}) — проверять было НЕКОГО, "
              "и это не «долгов нет»")
    else:
        print(f"✅ активных ролей {len(active)} из {total_roles} — читают все")
    report_out_of_window()
    sys.exit(0)
