...
это руками при остановке — и назавтра оно уже неправда.
    ...
    Этот блок НЕ ХРАНИТСЯ НИГДЕ. Он пересобирается при каждом чтении слепка —
    и потому протухнуть НЕ МОЖЕТ ПО ПОСТРОЕНИЮ. Не дисциплиной. Устройством.

📏 ЗАМЕР, НА КОТОРОМ ВРЕЗКА СТОИТ (2026-08-08): слепки всех ролей — 260 КБ, 63 секции,
...
def machine_block(db_path, role: str) -> list:
    ...
    # ── СВОЙ СЛЕД: последняя записка старше слепка ──────────────────────────
    try:
        last = conn.execute(
    ...
    # ── СВОД: что изменилось после сохранения слепка ────────────────────────
    try:
