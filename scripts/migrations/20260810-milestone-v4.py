...
def main():
    ...
    print(f"набор под вехой: окно {lo or '(с начала)'}…{hi} по датам имён · "
          f"предыдущая отметка версии {step_set['prev'] or '—'} · "
          f"ожидается шагов {len(step_set['expected'])}")
    if step_set['ambiguous']:
        # ⚰️ до 04.09: «из сверки исключены неоднозначные хвосты» — и двойник прятал дырку
        # в середине (карточка #536, стенд @COORD ⓑ). Теперь двойники сверяются по ПОЛНОМУ имени.
    ...
    if step_set['missing']:
        print()
        print('⛔ НЕ ЗАПУСТИЛАСЬ: под вехой не хватает шагов — '
              f"{len(step_set['missing'])} из {len(step_set['expected'])}:")
        for name in step_set['missing']:
            print(f'   🔴 {name}')
        sys.exit('   Веха объявила бы версию, которой в базе нет. '
    ...
    fp = record_step(conn, VERSION,
                     "отметка версии v4 = адресат полем + основание правил + журнал с "
