...
def main():
    ...
    if args.list:
        ...
        for key, lock, ver, blen, basis, ckind, body in rows:
            # Надгробие узнаётся по ПЕРВОЙ строке, а не по слову «отозвано» где угодно в тексте:
            # живое правило вправе рассказывать про чужой отзыв, и по грубому признаку оно
            # молча попадало в мёртвые (мой собственный промах 08.08 10:39 UTC — 9 вместо 7).
            is_tomb = body.lstrip().upper().startswith(("⛔ ОТОЗВАН", "ОТОЗВАН"))
            if is_tomb:
                tomb += 1
            else:
                live += 1
