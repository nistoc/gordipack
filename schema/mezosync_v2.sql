-- Gordi Mezosync Schema v2.0
-- Создаёт структуру коммуникационной БД для группы агентов.
-- Использование: sqlite3 mezosync.db < mezosync_v2.sql
--
-- v2 (2026-07-25): догон шаблона до живого рантайма Фазы 4 (md-off).
--   База v1 (14 таблиц) сохранена как есть; ниже добавлен раздел «-- v2 …»
--   с 4 новыми таблицами + 1 VIEW и расширенным описанием секций phoenix.
...
CREATE TABLE IF NOT EXISTS phoenix (
    role        TEXT NOT NULL,
    -- section — свободный TEXT (CHECK-ограничения нет). 7 живых секций слепка роли:
    --   identity — кто эта роль (имя, зона ответственности, мандат)
    --   state    — текущее состояние работы на момент слепка
    --   plan     — чем занимается / ближайшие шаги
    --   history  — существенная история решений роли
    --   launcher — строка запуска роли (launcher-промпт)
    --   rebirth  — что делать первым при воскрешении
    --   sources  — источники правды (пути/файлы/каналы)
    section     TEXT NOT NULL,       -- см. перечень 7 секций выше
    body        TEXT NOT NULL,
...
-- schema_version принудительно поднимаем до 2.0 (OR REPLACE перекрывает
-- унаследованное '1.0' при накате v2 на существующую БД).
...
----------------------------------------------------------------------
-- ====================================================================
-- v2 (2026-07-25): догон до живого рантайма Фазы 4 (md-off)
-- ====================================================================
-- Живой рантайм C:\guts\.atlas\.mezosync\mezosync.db эволюционировал за
-- пределы v1: добавлены 4 таблицы + 1 VIEW и колонка messages.resolved.
-- DDL ниже снят 1:1 с живой БД и обёрнут в IF NOT EXISTS для идемпотентности.
----------------------------------------------------------------------

-- messages.resolved — живая messages получила эту колонку ALTER-ом;
-- VIEW messages_all (ниже) её селектит, поэтому догоняем и здесь.
-- NB: ADD COLUMN без IF NOT EXISTS (SQLite не поддерживает) — рассчитано на
-- свежую установку; при повторном накате на БД с колонкой строку пропустить.
ALTER TABLE messages ADD COLUMN resolved INTEGER DEFAULT 0;  -- 0 | 1

-- read_batches — одноразовые ACK-токены разрезанного токена чтения ленты.
-- Половинки токена печатаются в первой и последней строках вывода read-messages,
-- --ack склеивает их и двигает курсор; строка здесь фиксирует выданный батч.
...
-- role_status — heartbeat ролей (кто жив/чем занят на момент updated_at).
...
-- stats_log — динамика метрик мезосинка (периодические JSON-слепки).
...
-- messages_history — вынесенная ретро-импорт md-история (сообщения из
-- прежних sync.*.md, перелитые в БД при переходе на Фазу 4). Схема — зеркало
-- messages (+ resolved), но id НЕ AUTOINCREMENT: id сохраняются из импорта.
...
-- messages_all — объединённый вид messages + messages_history для сквозного
-- поиска по всей ленте (живой + исторической); колонка source метит источник.
