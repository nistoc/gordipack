...
> ✅ **РЕАЛИЗОВАНО (B1, 2026-07-25).** Таблицы `backlog` + `backlog_events` + `backlog_tests`
> влиты в схему (см. `schema/mezosync_v2.sql`) и наполнены на живом контуре Atlas (десятки задач).
> Инструмент — `scripts/backlog.py` ({add,list,show,status,comment}). Этот документ — исходный дизайн,
> оставлен как обоснование модели данных.
...
- Доска **задач** по ролям — в Перископе (`src/` + `srcclient/`): фильтр по роли и статусу,
  разворот задачи, критерий приёмки, лента событий.
- ⚠️ Ограничение ОСТАЛОСЬ ТЕМ ЖЕ и оно намеренное: просмотрщик читает базу **только на чтение**
  и в файл не пишет. Значит результат проверки «глазами пользователя» фиксируется НЕ из него,
  а: (вариант A) пользователь говорит COORD → COORD пишет результат; (вариант B) пользователь
  сам зовёт `backlog.py`. Экран для записи — за рамками. Предлагаю вариант A.
  📌 Довод против записи из просмотрщика прежний и не зависит от того, на чём он сделан:
  окно в базу, умеющее в неё писать, перестаёт быть окном.
- ⚰️ Здесь описывалась карточка в `viewer/index.html` — странице на sql.js. Страница УДАЛЕНА
  2026-08-10: читала 5 таблиц из 27 и не знала признака отзыва правил.
...
## 9. Решения владельца (2026-07-12 10:54) — ЗАФИКСИРОВАНО

1. **Область:** ✅ **per-role + `SHARED`**. `list --role X` показывает задачи роли X + SHARED.
2. **Авто-прогон `code`-тестов:** ✅ **на усмотрение агента** (когда сам видит необходимость),
   НЕ каждый тик и не только по прямому требованию — агент решает. `test-run` доступен всегда.
3. **Результат `user_ui`-теста:** ✅ **через COORD** (пользователь сообщает → COORD пишет `test-result`).
4. **Старт:** ✅ **начать с B1** (схема + `backlog.py` базовый CRUD). Тесты (B2) — следом.
   Миграция прозы — по мере касания (не разом).
