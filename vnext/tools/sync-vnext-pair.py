...
⛔ приведение путей ... НЕ делает, и это решение. У соседней пары (scripts) путь контейнера
                        вырезается в «<КОНТУР>» — там файлы из шаблона не запускают.
                        ЗДЕСЬ запускают: bite-all гоняет приёмки прямо из шаблона.
                        Подстановка заглушки сломала бы общий прогон. Долг про пути машины
                        в публичном шаблоне остаётся отдельным — он не решается переносом
```
...
RUNTIME = Path(r"C:\guts\.atlas\vnext-tools")
