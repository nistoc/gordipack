...
⛔ приведение путей ... в ИСПОЛНИМОМ коде НЕ делает — с 09.08 20:43 UTC оно НЕ НУЖНО:
                        ...
                        остаётся текстом, и с карточки #357 перенос файла, чей шаблон несёт
                        заглушку, идёт С ОБРАТНОЙ ПОДСТАНОВКОЙ корень→заглушка; не свёлся
...
def main() -> int:
    ...
    if a.apply and plan:
        # ═══ Карточка #357: форточка «настоящая правка ПОВЕРХ заглушки» ═══════════════
        # Файл с только-заглушечной разницей в plan не попадает (#297/#13), но файл,
        # где поверх заглушки легла НАСТОЯЩАЯ правка, — попадает, и слепой write_bytes
        # затёр бы «<КОНТУР>» путём одной машины. Перенос такого файла идёт с ОБРАТНОЙ
        # подстановкой корень→заглушка (точный обратный ход к только_заглушка: та же
        # форма корня, прямые косые). Если после подстановки путь машины в тексте
        # ОСТАЛСЯ (обратные косые и прочие написания к заглушке не сводятся — их
        # прямая проверка тоже не признаёт), файл НЕ переносится и назван вслух:
        # молча оставить его значило бы отдать потребителю путь одной машины.
        ...
        for n in plan:
            цель = tpl / n
            шаблон_с_заглушкой = (n in t and ЗАГЛУШКА in
                                  t[n].read_text(encoding="utf-8", errors="replace"))
            if шаблон_с_заглушкой:
