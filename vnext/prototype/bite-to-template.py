...
def main() -> int:
    ...
    исходные_байты = (исток / "мирный.py").read_bytes()
    копия_байты = копия.read_bytes() if копия.exists() else b""
    ожидаемые = исходные_байты
    for настоящий, метка in drift.PLACEHOLDERS:
        if настоящий:
            ожидаемые = ожидаемые.replace(настоящий.encode("utf-8"), метка.encode("utf-8"))
    случай("③г байты: копия = источник с подставленными заглушками, и ничего больше",
           копия_байты == ожидаемые,
           "совпало байт в байт" if копия_байты == ожидаемые
           else f"разошлось: у источника {len(исходные_байты)} б, у копии {len(копия_байты)} б, "
                f"удвоенных концов строк {копия_байты.count(bytes([13, 13, 10]))}")
    ...
    print("")
    print("=" * 78)
    print(f"РАЗЛИЧАЮЩИХ СЛУЧАЕВ {len(итог)}, из них ВСТРЕЧНЫХ 2 (③б и ⑤); ③г добавлен по находке @COORD")
