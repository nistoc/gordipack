...
# 🪤 ЗАМОК — НЕ ТОЛЬКО \w. Найдено контуром AIA, подтверждено в нашем коде (2026-09-13):
#    `locked_by` бывает ДАТИРОВАННЫМ («owner-2026-07-29» — export-rules.py печатает его как
#    есть, `🔒{locked}`), а дефис в \w не входит. Правило с таким замком не совпадало с
#    заголовком ВООБЩЕ — то же ложное красное «ЕСТЬ В БАЗЕ, НЕТ В ФАЙЛЕ», от которого уже
#    лечили сам ключ. [\w:.-] берёт дефис, точку и двоеточие — формы замков, встреченные
#    в живых правилах (`owner`, `coord`, `owner-2026-07-29`), не сужая ключ жёстче необходимого.
