# -*- coding: utf-8 -*-
"""
bite-task.py — укус критерия «готово» и связи записки с задачей (Ш5, пункт 2.7).
...
Проверяется не «пишется ли строка», а четыре вещи, ради которых шаг делается:
  ...
  ③ сторож закрытия не принимает пустую строку за критерий — иначе он украшение;
...
def run():
    ...
    card(con3, 13, title="с критерием", done_when="укус 15/15 и 6 мутантов пойманы")
