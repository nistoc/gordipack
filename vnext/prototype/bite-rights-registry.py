...
BREAKS = {
    ...
    # 🩸 порча AIA 01 (карточка #644, предложение 01): откат образца заголовка к форме
    #    БЕЗ номера раздела — ровно та беда, из-за которой у AIA права одной роли из
    #    восьми не проверялись молча.
    "section-number": ('RIGHTS_HEADER = re.compile(r"§\\s*\\d*\\s*ПРАВА", re.I)',
...
def make_stand(tmp, name, memory, rights):
    ...
    con.execute("CREATE TABLE role_rights (id INTEGER PRIMARY KEY, role TEXT, "
                "revoked_at TEXT)")
    # 🩸 добавлено 05.09 08:14 UTC: проверка стала спрашивать и СВОД ПРАВИЛ — чтобы
    # отличать «записи нет вовсе, источник — правило свода» от «ни ссылки, ни пометки»
    # (по слову @COORD, записка #4810 §②). Стенд без этой таблицы даёт код 2
    # «НЕ ПРОВЕРЕНО», и ВСЕ случаи краснеют по посторонней причине — что и случилось.
...
def main() -> int:
    ...
    try:
        ...
        case("⑰-бис встречный: запрет в ТОЙ ЖЕ форме таблицы не судится вовсе",
             ...
             "расширив отбор, легко захватить запреты — тогда их нечем закрыть")

        # ⑰-тер ЗАПРЕТ, У КОТОРОГО РАЗРЕШИТЕЛЬНОЕ СЛОВО СТОИ́Т ПОЗЖЕ.
        # 🩸 Заведён по находке @PROTO (записка #4831 §③), и находка её была точной:
        # случай ⑰-бис выше НЕ ПРОВЕРЯЕТ ТОГО, ЧТО ОБЕЩАЕТ. Его строки «⛔ НЕТ |
        # rule8-destructive…» не несут НИ ОДНОГО разрешительного слова ⇒ отсеиваются
        # по «разрешения нет вовсе», а не по «запрет высказался первым». Обезвредь
        # запретительную примету целиком — он останется зелёным, и молча.
        # ⚡ Это ДОСЛОВНО мой оплаченный класс (карточка #520): случай зелен ПО ПОСТОРОННЕЙ
        ...
        case("⑳ печатается НАЙДЕНО · рассмотрено · отсеяно С ПРИЧИНОЙ, а не одно число",
             ...
             and "обязанность, а не право: 1" in out and "запрет или строка без разрешения: 1" in out,
             "замер STUD: «отсеяно 88» читается как «проверка не смотрит ни на что», "
             "а незаконным был отсев четырёх строк")

        # ㉑ ПРЕДЛОЖЕНИЕ AIA 01 (карточка #644): заголовок с НОМЕРОМ раздела распознаётся.
        # Живой пример из памяти роли: «## §2 ПРАВА И ЗАПРЕТЫ РОЛИ» — прежний образец
        # искал «§ПРАВА» ВПЛОТНУЮ и такую строку не находил вовсе; роль выпадала молча.
        db = make_stand(tmp, "numbered-header", [("ZZK", "identity",
            "[заведено COORD]\n\n## §2 ПРАВА И ЗАПРЕТЫ РОЛИ\n\n"
            "| Действие | Разрешено? | Источник |\n|---|---|---|\n"
            "| поднять службу :7777 | ✅ БЕЗ спроса | стенд общий |")], RIGHTS)
        out, code = run(db, guard)
        case("㉑ заголовок с НОМЕРОМ раздела «## §2 ПРАВА И ЗАПРЕТЫ РОЛИ» — таблица найдена",
             code == 1 and "ZZK" in out,
             "образец расширен до §\\s*\\d*\\s*ПРАВА (предложение AIA 01, карточка #644)")

        # ㉒ встречный: «§ ПРАВА» (пробел, без номера) — видится КАК ПРЕЖДЕ
        db = make_stand(tmp, "spaced-header", [("ZZL", "identity",
            "## § ПРАВА\n\n| Действие | Разрешено? | Источник |\n|---|---|---|\n"
            "| что-то | ✅ ДА | стенд общий |")], RIGHTS)
        out, code = run(db, guard)
        case("㉒ встречный: «## § ПРАВА» (пробел, без номера) — по-прежнему видится",
             code == 1 and "ZZL" in out)

        # ㉓ встречный: строка «права роли» БЕЗ знака § — заголовком не считается
        db = make_stand(tmp, "no-sign", [("ZZM", "identity",
            "права роли\n\n| что-то | ✅ ДА | стенд |")], RIGHTS)
        out, code = run(db, guard)
        case("㉓ встречный: «права роли» без знака § — не заголовок, таблица не ищется",
             code == 0 and "ZZM" not in out)
        ...
        if not a.break_key:
            ...
            db = make_stand(tmp, "p-section-number", [("ZZK", "identity",
                "[заведено COORD]\n\n## §2 ПРАВА И ЗАПРЕТЫ РОЛИ\n\n"
                "| Действие | Разрешено? | Источник |\n|---|---|---|\n"
                "| поднять службу :7777 | ✅ БЕЗ спроса | стенд общий |")], RIGHTS)
            run_break("section-number", db, "рассмотрено: 0")
