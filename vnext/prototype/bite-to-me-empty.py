...
def main():
    ...
    ok &= case("① отбор отвечает, когда обращённое есть",
    ...
    ok &= case("⑤ отметка прочитанного у головы: сказано, что и в ленте пусто",
    ...
    con.close()
    ok &= case("⑥ отметка прочитанного НЕ двинута и ключа подтверждения нет",
               moved == 1 and "--ack" not in out6,
               f"отметка прочитанного осталась {moved}; ключ в выводе: {'есть' if '--ack' in out6 else 'нет'}",
               differ=True)
    ...
    ok &= case("⑦ обычное чтение на пустой ленте говорит «Нет непрочитанных сообщений»",
               ...
               "правка экрана не тронула обычное чтение — предметы разные", differ=True)
