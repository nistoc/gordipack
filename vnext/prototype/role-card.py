    ...
    for d in targets:
        ...
        if a.check:
            if not f.exists():
                ...
                # ⚖️ Причин ТРИ, и различить их сверка не может — она видит только
                # «файл ≠ свежесобранный текст». Третью нашла @CHROME (#3718 ②) на себе:
                # после правки ПОКАЗА сверка покраснела у всех, кто уже разложил карточку,
                # а текст называл две причины — читающий пошёл бы искать чужую правку,
                # которой не было.
                print(f"🔴 {d.name}: файл РАСХОДИТСЯ с тем, что собрала бы база СЕЙЧАС. "
                      f"Причины три: правили файл рукой · база ушла вперёд · "
                      f"СБОРЩИК стал собирать иначе")
                print("   рукой не правили — это не поломка, а устаревший файл: пересобери --write")
                bad += 1
            else:
