...
⚠️ ЭТО КОПИЯ В ПЕСОЧНИЦЕ (карточка #618, возврат PROTO). Отличия от живой
vnext-tools/bite-rules-mirror.py — ТОЛЬКО два, оба явно помечены ниже:
  · добавлен `--checker <путь>` — чем испытывать (по умолчанию прежнее поведение:
    пакетная копия vnext/prototype/check-rules-mirror.py), нужен, чтобы прогнать
    ОДНУ и ту же приёмку и против единой редакции, и против живой копии;
  · случай ⑥ — ожидание переведено под критерий ④ карточки #618 (см. комментарий
    прямо над ним). Больше НИЧЕГО не менялось — имена уже были английские, переводить
    было нечего.
...
СЕМЬ СЛУЧАЕВ, ТРИ ИЗ НИХ ВСТРЕЧНЫЕ (без них «зелено» ничего не стоит):
    ...
    ⑥ файла нет, правила есть  «сверить НЕЧЕМ» — отказ, а не «чисто» (карточка #618 ④)
...
ЗАПУСК: python bite-rules-mirror.py [--checker <путь к check-rules-mirror.py>]
...
def main() -> int:
    ...
    # 🆕 --checker — ЕДИНСТВЕННОЕ содержательное добавление к живой приёмке (карточка #618,
    # возврат PROTO): позволяет прогнать ОДНУ И ТУ ЖЕ приёмку и против единой редакции
    # (scripts-new/vnext-new), и против живой копии — без правки исходника между прогонами.
    # По умолчанию — прежнее поведение (пакетная копия vnext/prototype/), чтобы прогон
    # БЕЗ флага остался тем же самым испытанием, что было.
    ap.add_argument("--checker", default=None,
                    help="путь к check-rules-mirror.py; по умолчанию — "
                         "vnext/prototype/check-rules-mirror.py из REPO")
    a = ap.parse_args()
    ...
    try:
        ...
        checker = Path(a.checker) if a.checker else REPO / "vnext" / "prototype" / "check-rules-mirror.py"
        ...
        mirror.unlink()
        r = run(sys.executable, checker, "--db", db, "--file", mirror)
        # ⚠️ КАРТОЧКА #618 СМЕНИЛА ОЖИДАНИЕ ЗДЕСЬ НАМЕРЕННО (2026-09-14, возврат PROTO).
        # Было: код 2, текст «не нашёл файл» — так отвечала СТАРАЯ vnext-tools-копия;
        # комментарий рядом называл это «третьим исходом, лучше критерия». Единая редакция
        # (scripts-new/vnext-new check-rules-mirror.py) для ОБЕИХ живых копий и пакета
        # отвечает на «файла нет, правила есть» кодом 1 и текстом «сверить НЕЧЕМ» — это
        # поведение старой .mezosync/scripts-копии, и карточка #618 (критерий ④) сделала
        # его каноном явно, а не по недосмотру. Ожидание здесь переведено ПОД критерий,
        # не вслепую: см. отчёт G6/#618 — этот же случай ЗЕЛЁН на единой редакции
        # (bite-rules-mirror-vs-unified.txt) и ОБЯЗАН провалиться на старой живой
        # vnext-tools-копии без флага --checker (bite-rules-mirror-vs-live.txt) — разница
        # и есть доказательство, что правка ожидания не пустая.
        say("⑥", r.returncode == 1 and "сверить" in r.stdout.lower() and "нечем" in r.stdout.lower(),
            "файла нет при живых правилах — «сверить НЕЧЕМ» (код 1), критерий ④ карточки #618")
