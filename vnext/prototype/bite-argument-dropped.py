...
Случаи (различающий = сторож обязан ответить ИНАЧЕ, а не одинаково):
  ① довод ПОТЕРЯН → находка      контроль: проверка вообще умеет краснеть
...
def main() -> int:
    ...
    ok &= case("① довод потерян — находка есть (проверка умеет краснеть)",
    ...
    ok &= case("④ ЗАГЛУШКА рядом не перекрывает настоящую сигнатуру",
               "во всех прямых вызовах довод передан" in out,
               "у запасного определения доводов нет; меря по нему, проверка обвинила "
               "пять ВЕРНЫХ вызовов — этот случай охраняет починку", differ=True)
