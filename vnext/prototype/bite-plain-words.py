...
def shown_lines(src: str) -> tuple[set[int], dict[int, list[str]]]:
    """Номера строк с ПОКАЗЫВАЕМЫМ текстом + литералы внутри {…}-подстановок по строкам
    (карточка #592) — второе судится отдельно от текста строки, потому что имя переменной
    внутри {…} судить нельзя (граница ⑩), а литерал внутри той же {…} — можно и нужно."""
    try:
        tree = ast.parse(src)
    except SyntaxError:
        return set(), {}
    ...
    return out, _литералы_подстановок(tree)
...
def scan(root: Path) -> tuple[list[str], int, int]:
    ...
    for p in sorted(root.rglob("*.py")):
        ...
        for i, line in enumerate(src.splitlines(), 1):
            ...
            судимое = _без_имени_цели(_без_подстановок(судимое))
            # ═══ Карточка #592: _без_подстановок вырезала {…} ЦЕЛИКОМ — вместе с именем
            # (которое судить нельзя, граница ⑩) вырезался и строковый литерал внутри той
            # же подстановки (который судить нужно). Литералы, отдельно найденные ast-ом,
            # возвращаются в судимый текст здесь — имена в них уже не попадают ни разу.
            if i in литералы:
