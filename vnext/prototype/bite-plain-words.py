...
PATTERN_AND_PROBES = [
    ...
    # ═══ Карточка #611 (проверка прежних слов): БЕЗ ГРАНИЦЫ СЛЕВА образец «ворот[аыу]\w*»
    # попадал ВНУТРЬ обычных русских слов — «раз-ворот-а», «по-ворот-а», «пере-ворот-а»
    # (падежные формы «разворот»/«поворот»/«переворот» на «-а»): у приставки перед «ворот»
    # не было отсечки. Живой сосед в этом же каталоге: bite-backup-restore.py несёт слово
    # «разворота» в шапках/комментариях (сегодня печатаемого текста не красит — комментарии
    # эта проверка не судит, — но полагаться на «пока не доехало до печати» нельзя: образец
    # ищет по ЛЮБОЙ печатаемой строке, а не по сегодняшнему состоянию файлов).
    # ⚖️ Флаг регистра у ЭТОГО образца — re.I (`INVENTED = re.compile(..., re.I)` ниже):
    # заглавные буквы уже покрыты им, отдельная заглавная группа в границе не нужна.
    # «гейт» границу получает ТОЙ ЖЕ формой из симметрии, хотя живых столкновений внутри
    # другого слова для него не найдено (поиск по обоим каталогам — .mezosync/scripts
    # и vnext-tools — 2026-09-14): пустой встречный случай честнее тихой дыры, которую
    # никто не искал.
    (r"(?<![а-яё])гейт\w*|(?<![а-яё])ворот[аыу]\w*", ["гейт", "ворота"]),
...
def main() -> int:
    ...
    # 🩸 КАРТОЧКА #621 ②. Решено: хранилище резервных копий (atlas.agents-sync.db) тоже
    # судится этой приёмкой. Довод: restore-verify.py читает человек БЕЗ роли и БЕЗ
    # контура — просто `git clone` + `python restore-verify.py`, см. RESTORE.md того
    # репозитория, — а «роль читает вывод и переписывает слова в свою память», ради
    # чего заведено само правило, тут применимо буквально так же, как к .mezosync/scripts
    # и vnext-tools. Прежде каталог не смотрелся вовсе — не по решению, а по недосмотру:
    # ЗАМЕР 2026-09-14 08:27 UTC (карточка #610) нашёл там три запрещённых слова и
    # «СВЕРКА ЗЕЛЁНАЯ» печатаемым текстом при зелёной этой самой приёмке. Отсутствие
    # каталога — не отказ: у копии этого инструмента ВНЕ Atlas (например, в пакете) его
    # нет и не будет — это Atlas-специфичный репозиторий, не часть шаблона.
    mirror = mezo_paths.container_root(__file__) / "atlas.agents-sync.db"

    # ── ① ЖИВОЙ ВЫВОД ЧИСТ ────────────────────────────────────────────────────
    hits_live, f1, l1 = scan(live)
    hits_tools, f2, l2 = scan(tools)
    hits_mirror, f3, l3 = scan(mirror) if mirror.is_dir() else ([], 0, 0)
    hits = hits_live + hits_tools + hits_mirror
    ok &= case("① показываемый текст инструментов чист от выдуманных слов"
               + (" (включая хранилище резервных копий)" if mirror.is_dir() else ""),
               not hits,
               (f"смотрено файлов {f1 + f2 + f3}, показываемых строк {l1 + l2 + l3},"
                f" находок {len(hits)}"
    ...
    # ── ㉔ КАРТОЧКА #611: ЛЕВАЯ ГРАНИЦА У «ворота» (и «гейт») ──────────────────
    # 🩸 Найдено при проверке прежних слов 2026-09-14: образец «ворот[аыу]\w*» БЕЗ
    # ГРАНИЦЫ СЛЕВА попадал ВНУТРЬ обычных русских слов — «раз-ворот-а», «по-ворот-а»,
    # «пере-ворот-а» — и красил бы обычную речь про разворот/поворот/переворот, требуя
    # её калечить. Жанр случая — тот же, что у ⑭/⑭-бис/⑭-тер: база, встречный, обратный
    # ход, только граница здесь не про имя переменной (⑩), а про приставку слева от слова.
    d24 = mezo_stand.new("bite-words-gate-")
    (d24 / "probe_gate.py").write_text(
        '# -*- coding: utf-8 -*-\n'
        'def main():\n'
        '    print("итог: ворота открыты")\n'
        '    print("итог: Ворота открыты")\n'
        '    print("итог: прошли через ворота")\n',
        encoding="utf-8")
    gate_hits, _, _ = scan(d24)
    # Заголовок случая называет слово НЕ дословно: сама эта строка печатается, и дословное
    # слово в ней было бы находкой случая ① (поймано решающим прогоном PROTO 14.09 — прогон
    # исполнителя судил прежнюю копию файла, а не новую).
    ok &= case("㉔ прежнее слово о прогоне проверок (три формы: строчная, заглавная, после "
               "предлога) в печатаемом тексте — судится",
               len(gate_hits) == 3,
               f"находок {len(gate_hits)} из 3 — голое слово вне ловушки-приставки должно "
               "ловиться независимо от регистра и соседей по строке", differ=True)

    # ㉔-бис ВСТРЕЧНЫЙ: приставочные слова «разворота·поворота·ПЕРЕВОРОТА» — обычная
    # русская речь, а не жаргон о полном прогоне проверок; без границы слева образец
    # попадал внутрь них.
    (d24 / "probe_gate.py").write_text(
        '# -*- coding: utf-8 -*-\n'
        'def main():\n'
        '    print("итог: после разворота техника легла на курс")\n'
        '    print("итог: до поворота ещё три километра")\n'
        '    print("итог: ждём ПЕРЕВОРОТА рынка")\n',
        encoding="utf-8")
    gate_quiet, _, _ = scan(d24)
    ok &= case("㉔-бис ВСТРЕЧНЫЙ: «разворота·поворота·ПЕРЕВОРОТА» — НЕ находка",
               not gate_quiet,
               f"находок {len(gate_quiet)} — без левой границы образец «ворот[аыу]\\w*» "
               "попадал внутрь этих слов; регистр учтён тем же флагом, что и у всего "
               "образца (re.I) — отдельная заглавная группа в границе не нужна", differ=True)

    # ㉔-тер ОБРАТНЫЙ ХОД: снять ИМЕННО левую границу у «ворота» → встречные случаи
    # обязаны провалиться, и именно по СВОЕЙ причине (находка на «разворота» и т.п.),
    # а не молчанием. Разница двух прогонов и есть починка.
    unbounded_pattern_text = whole_pattern_text.replace(
        r"(?<![а-яё])ворот[аыу]\w*", r"ворот[аыу]\w*", 1)
    whole_pattern4 = globals()["INVENTED"]
    globals()["INVENTED"] = re.compile(unbounded_pattern_text, re.I)
    try:
        gate_broken, _, _ = scan(d24)
    finally:
        globals()["INVENTED"] = whole_pattern4
    broken_on_traps = [h for h in gate_broken
                        if "разворота" in h or "поворота" in h or "ПЕРЕВОРОТА" in h]
    ok &= case("㉔-тер ОБРАТНЫЙ ХОД: без левой границы встречные случаи проваливаются "
               "по своей причине",
               unbounded_pattern_text != whole_pattern_text and len(broken_on_traps) == 3,
               f"снятие границы обязано уронить ИМЕННО случаи ㉔-бис (разворота/поворота/"
               f"ПЕРЕВОРОТА), а не что-то постороннее — провалов по своей причине "
               f"{len(broken_on_traps)} из 3"
               + ("" if len(broken_on_traps) == 3
                  else ": " + "; ".join(broken_on_traps) if broken_on_traps
                  else " — обратный ход ничего не сломал, значит граница проверена не той"
                       " заменой"), differ=True)
    mezo_stand.release(d24)
    ...
    # ── ㉕ КАРТОЧКА #621 ②: main() ДЕЙСТВИТЕЛЬНО зовёт scan() на хранилище ────
    # резервных копий, а не только упоминает его имя. Статически (по своему исходнику,
    # приём как у С8(в) bite-backup-restore.py) + динамически (probe-копия с тем же
    # словом, что нашла карточка #621, — «рубеж схемы» — не живой файл).
    own_text = Path(__file__).resolve().read_text(encoding="utf-8")
    wired = 'atlas.agents-sync.db"' in own_text and "scan(mirror)" in own_text
    ok &= case("㉕ main() действительно зовёт scan() на хранилище резервных копий"
               " (проверено по своему исходнику)",
               wired,
               "ищем имя каталога и вызов scan(mirror) в этом же файле — иначе каталог "
               "мог бы упоминаться, ничего не разбирая", differ=True)

    if mirror.is_dir():
        ok &= case("㉕-а хранилище резервных копий участвует в реальном прогоне (не"
                   " «неприменимо»)",
                   True, f"каталог {mirror} найден и разобран случаем ① выше"
                   f" (файлов {f3}, строк {l3})", differ=True)
    else:
        case("㉕-а хранилище резервных копий рядом отсутствует — законное «неприменимо»,"
             " не отказ", True,
             f"каталога {mirror} нет (копия этого инструмента вне Atlas — например, "
             "в пакете — его не несёт)", differ=True)

    d25 = mezo_stand.new("bite-words-mirror-")
    (d25 / "probe_mirror.py").write_text(
        '# -*- coding: utf-8 -*-\n'
        'def main():\n'
        '    print("рубеж схемы: 1")\n',
        encoding="utf-8")
    mirror_probe_hits, _, _ = scan(d25)
    # ⚠️ Заголовок случая НЕ цитирует само слово — иначе печатаемый текст ЭТОЙ приёмки
    # нарушал бы то же правило, которое она стережёт (класс «приёмка отпустила саму
    # себя», уже названный в шапке файла). Слово стоит только в комментарии выше и в
    # PLANTS-строке probe-файла (аргумент .write_text — не print, не судится).
    ok &= case("㉕-бис нарочная проба: слово из находки карточки #621 в probe-копии"
               " хранилища scan() находит",
               any("probe_mirror.py" in h for h in mirror_probe_hits),
    ...
    ok &= case("⑤ контроль: приёмке было на что смотреть",
               f1 + f2 + f3 > 30 and l1 + l2 + l3 > 500,
               f"файлов {f1 + f2 + f3}, показываемых строк {l1 + l2 + l3} — без этого "
               f"случая зелёный мог бы означать «ничего не нашлось, потому что ничего "
               f"не искали»")
