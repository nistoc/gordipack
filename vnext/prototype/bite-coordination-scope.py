...
def main() -> int:
    ...
    if ОБРАЗЕЦ is not None:
        with tempfile.TemporaryDirectory(prefix="bite-coord-carrier-") as tmp2:
            контур2 = pathlib.Path(tmp2) / "контур"
            r2 = subprocess.run([sys.executable, str(ОБРАЗЕЦ / "scripts" / "init-group.py"),
                                 "--name", "bite", "--path", str(контур2 / ".mezosync"),
                                 "--roles", "COORD"], capture_output=True, text=True,
                                encoding="utf-8", errors="replace", timeout=900)
            if r2.returncode == 0:
                shutil.copy(СТОРОЖ, контур2 / ".mezosync" / "scripts" / "guard-all.py")
                объявление = контур2 / ".mezo-no-coordination"

                # ⑪ ДОЛГИЙ НОСИТЕЛЬ: каталогов нет, объявление лежит ФАЙЛОМ.
                for п in найти(контур2) + найти_мост(контур2):
                    shutil.rmtree(п)
                объявление.write_text("", encoding="utf-8")
                файлом = прогон(контур2)
                стр11 = строка(файлом, "замороженные md")
                ok &= case("⑪ объявление ФАЙЛОМ переживает вызов — жёлтое, носитель назван",
                           not стр11.startswith("⛔") and "НЕ ПОСТАВЛЕНА" in файлом
                           and ".mezo-no-coordination" in файлом,
                           f"строка: {(строка(файлом, 'НЕ ПОСТАВЛЕНА') or '(нет)').strip()[:110]}",
                           differ=True)

                # ⑫ ОБЪЯВЛЕНИЕ ПЕРЕЖИЛО ПРИЧИНУ: каталог вернулся, файл лежит.
                (контур2 / "coordination").mkdir(exist_ok=True)
                спор = прогон(контур2)
                ok &= case("⑫ каталог НАЙДЕН, а объявление лежит — «спорит с находкой»",
                           "СПОРИТ С НАХОДКОЙ" in спор,
