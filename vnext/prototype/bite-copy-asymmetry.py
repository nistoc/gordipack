...
def main() -> int:
    ...
    ok &= case("③ общий файл с разным содержимым — красное «ДРЕЙФ»",
               ...
               "копии разъезжаются молча — это и есть предмет проверки", differ=True)
    ...
    #    Шаблонный текст строится через mezo_stand.crlf_twin (карточка #626) — общий
    #    приём вместо самодельного литерала. Признак CRLF — часть условия ЭТОГО же
    #    случая (не отдельный assert: тот прервал бы прогон трассировкой и отключается
    #    под python -O) — испытуемый нормализует оба конца перед сравнением, и «мнимый»
    #    CRLF (поломанный crlf_twin, вернувший текст как есть) дал бы тот же зелёный
    #    ответ по ДРУГОЙ причине — байты проверяются напрямую, не только через исход.
    out, code = stand(tmp, "four", {"a.py": "print(1)\n"},
                      {"a.py": mezo_stand.crlf_twin("print(1)\n")})
    four_bytes = open(os.path.join(tmp, "four", "template", "a.py"), "rb").read()
    four_crlf_hits = four_bytes.count(b"\r\n")
    ok &= case("④ разный перевод строки дрейфом НЕ считается (встречный к ③)",
               code == 0 and "ДРЕЙФ" not in out and four_crlf_hits > 0,
               f"код {code}; стенд несёт CRLF: байтов \\r\\n {four_crlf_hits} (crlf_twin) — "
               f"иначе красное вечно и по невиновной причине, git правит перевод строки сам",
               differ=True)
