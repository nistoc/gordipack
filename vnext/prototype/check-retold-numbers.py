...
# 🩸 ЧИСЛО С РАЗРЯДНЫМ ПРОБЕЛОМ — ОДНО ЧИСЛО (PROTO 2026-09-25 15:20 UTC). Живая строка
#    моей памяти «тринадцать правил 122 606 → 34 229 знаков» резалась до «правил 122»,
#    и размер в знаках красился как пересказ числа правил — две ложные находки из двух.
#    Гаситель — только справа от числа: живой экземпляр есть лишь у этой стороны
#    (правило этого файла: гасители — по живому материалу, не по воображению).
WHOLE_AFTER = r"(?!\s\d{3}(?!\d))"
RULES_N = re.compile(rf"\bправил\b{ADJ}(\d{{1,4}})\b{WHOLE_AFTER}|\b(\d{{1,4}})\s+правил\b", re.I)
