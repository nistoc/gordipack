...
def mezo_root(script_file) -> Path:
    ...
    sys.exit(
        ...
        f"     ⚠️ ЗА ЭТИМ ЗАМКОМ ЕСТЬ ВТОРОЙ, и второй выход до него не довозит.\n"
        f"     Многим инструментам нужен не только файл базы, но и КАТАЛОГ ГРУППЫ —\n"
        f"     сами они его не просят, а просит их сосед по каталогу. Тогда следом\n"
        f"     придёт отказ «контейнер группы НЕ НАЙДЕН», и там --db не поможет.\n"
        f"     👉 Если не уверены — берите первый выход: он снимает оба.\n"
        f"     📏 Замер @COORD 2026-09-06: 176 инструментов из 243 (72%) упрутся во второй."
    )
