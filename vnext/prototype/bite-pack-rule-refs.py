...
пропал) — проверка красна, названо поимённо:
  ...
  · zz-annex — ТОЖЕ самодостаточная ТЕСТОВАЯ ФИКСТУРА, того же класса, что выше: строка
    bite-set-rule-revoke.py:283 (случай ⑯: «--show называет приложение, когда оно есть»)
    сравнивает вывод инструмента с литералом «--key zz-annex --annex» — своей ЖЕ командой,
    которой строкой раньше сама создала приложение rules-annex/zz-annex.md в ИЗОЛИРОВАННОМ
    временном контейнере той приёмки. Ключ «zz-annex» нигде не адресован роли и в rules/
    пакета не нужен: имя с префиксом zz- — сквозная примета test-only ключей ВСЕГО файла
    (zz-mixed, zz-noannex, zz-no-such-rule, zz-owner, zz-agree рядом) — префикс здесь ТОЛЬКО
    пояснение для читателя, граница проведена именем файла и номером строки, а не угадана
    по префиксу regex'ом.
"""
...
KNOWN_OPEN = {"md-to-sqlite-phased-cutover", "core-service-start-and-migrations", "zz-annex"}
