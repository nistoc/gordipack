...
WORDS = {
    ...
    # ═══ Карточка #611: ГРАНИЦА СЛЕВА добавлена обоим — без неё «ворот[аыу]\w*» попадал
    # ВНУТРЬ «разворота»/«поворота»/«переворота» (обычная речь, не жаргон о прогоне).
    # `ANY = re.compile(..., re.I)` ниже — регистр уже покрыт флагом, заглавная группа
    # в границе не нужна. «гейт» — той же формой из симметрии (живых столкновений внутри
    # другого слова не найдено поиском по обоим каталогам 2026-09-14, но пустая граница
    # честнее непроверенной дыры).
    "гейт": r"(?<![а-яё])гейт\w*",
    "ворота": r"(?<![а-яё])ворот[аыу]\w*",
