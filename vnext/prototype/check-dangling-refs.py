...
EXPLAIN = re.compile(
    ...
    r"|релиз\w*|верси\w*|пункт\w*|строк\w*|порт\w*|тикет\w*|issue|PR|MR)\s+#?\d{1,5}",
    re.I)
...
def report(text: str, where: str, quiet: bool = False) -> int:
    ...
    for line, col, ref, around in hits:
        ...
        print(f"      …{around}…")
    print("\n   📌 Это ПРЕДУПРЕЖДЕНИЕ, а не отказ (слово владельца 13:01 UTC). Но чинится оно")
    print("      только СЕЙЧАС: задним числом ссылки не переписываем, старое не трогаем.")
    return len(hits)
