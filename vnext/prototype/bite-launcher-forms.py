...
def main() -> int:
    ...
    blind, seen, unjudged, nothing, unrunnable, unaffected = [], 0, [], [], [], []
    for name in targets:
        ...
        if not (свои_ok["ok"] or свои_ok["red"] or свои_ok["skip"]):
            ...
            перемер = ответ_изменился(name, scripts_ok, db_ok, scripts_i, db_i)
            if перемер is False:
                unaffected.append(name)
                mark = ("⚪ поломка ответ форм НЕ меняет (прямой прогон совпал) — "
                ...
                seen += 1
                blind.append(name)
                mark = ("🔴 СЛЕП: прямой прогон показал, что ответ ИЗМЕНИЛСЯ"
    ...
    def стенд_с_формой(механизм, хвост):
    ...
    def пара_стендов(механизм, хвост):
        """Целый и сломанный стенды с ЕДИНСТВЕННОЙ подсаженной формой механизма."""
        цел = стенд_с_формой(механизм, хвост)
        слом = стенд_с_формой(механизм, хвост)
        f_ = слом[0] / механизм
