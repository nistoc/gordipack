...
def main() -> int:
    ...
    try:
        ...
        # 🩸 НАЙДЕНО COORD (разбор PROTO, задача D, замер 2026-09-13): копия несёт ещё и ЖИВЫЕ
        # объявления о правке инструментов (tool_leases) — MEZO_LEASE_TEST=1 выше заставляет
        # lease.check() судить их и на копии тоже (иначе приёмку самого механизма аренды не
        # на чем ставить, см. lease.py TEST_ENV). Пока где-то в живом контуре держится
        # объявление о правке backlog.py, эта же строка едет в копию и красит случаи ②③④,
        # хотя код backlog.py цел: приёмка мерила чужую занятость, не механизм claim.
        # Снимаем ЗДЕСЬ, В КОПИИ (НИКОГДА не в живой базе): пустая занятость нужна для
        # чистого случая, ровно как и у backlog_events строкой выше. Подтверждено замером:
