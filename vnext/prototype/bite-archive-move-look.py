...
Случаи (различающий = read-phoenix обязан ответить ИНАЧЕ, а не одинаково):
  ...
  ④ правка мимо инструмента, ПОТОМ штатный перенос поверх нее → 🔴 остаётся:
     перенос НЕ ОТМЫВАЕТ обход, случившийся ДО него                       РАЗЛИЧАЮЩИЙ
  ...
  ⑥ НАРОЧНАЯ ПОЛОМКА (archive_move_look() отключена в копии инструмента):
     красятся РОВНО ①②⑤ (там, где правку спасала функция), а ③④ остаются
     как были — 🔴 у них не от функции, поломка их не касается             РАЗЛИЧАЮЩИЙ
...
case("⑥ поломка КРАСИТ случай ① (раньше его спасала функция)", "СТАРШЕ текста" in b_out1)
case("⑥ поломка КРАСИТ случай ② (раньше его спасала функция)", "СТАРШЕ текста" in b_out2)
case("⑥ поломка КРАСИТ случай ⑤ (живой ING/plan, раньше спасала функция)", "СТАРШЕ текста" in b_out5)
case("⑥ поломка НЕ ТРОГАЕТ случай ③ (он и без функции был 🔴)", "СТАРШЕ текста" in b_out3)
case("⑥ поломка НЕ ТРОГАЕТ случай ④ (он и без функции был 🔴)", "СТАРШЕ текста" in b_out4)
