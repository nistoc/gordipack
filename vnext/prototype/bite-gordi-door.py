...
# ПРИЧИНА ①/⑧ ПЕРЕМЕРЕНА ПО КОДУ (14.09; не гипотеза, а проверено диффом на живой базе):
# сам ОБЩИЙ ЗАПУСК (gordi.py, функция main) ничего не меняет — он зовёт
# `subprocess.run([sys.executable, str(t[name]), *rest], env=env)` БЕЗ capture_output,
# тем же файлом, тем же интерпретатором, той же средой, и возвращает код инструмента как
# есть (gordi.py, строки 122-126). Расходится не ОН, а `backlog.py list`: подсказка
# «печатается роли один раз» (mezo_hints.py, карточка #586) — первый показ ПОЛНЫЙ текст
# (у backlog.py это CRITERION_HIDDEN_TEXT под ключом "backlog-list-full", строки 810-813),
# второй и далее — короткая строка-ссылка `ℹ️ подсказка «ключ» показана ЧАС; полный текст —
# добавь --full` (mezo_hints.подсказка, строки 150-151). Прямой вызов и вызов через дверь —
# ДВА ПОСЛЕДОВАТЕЛЬНЫХ обращения к ОДНОЙ И ТОЙ ЖЕ копии базы одной и той же ролью: первое
# получает полный текст, второе — уже короткую ссылку, а сама подсказка НЕ шумовая метка
# времени (её нет в NOISE выше: час там БЕЗ секунд, `%H:%M`). Перемерено дифом на
# сохранённом стенде провального прогона: прямой вызов и вызов через дверь разошлись
# РОВНО одной строкой — полный текст против короткой ссылки того же ключа
# "backlog-list-full"; отметка `hint_seen` для (PROTO, backlog-list-full) стёрта и разбор
# повторён — разошлись СНОВА, той же парой строк, слово в слово (весь остальной вывод
# совпал байт в байт).
# ⇒ чинится НЕ запуск (он ни при чём), а стираются ПОИМЁННО оба известных вида подсказки —
# не любая отличающаяся строка вслепую.
HINT_FULL = re.compile(r"ℹ️ срез критерия у карточек скрыт — полный вид: backlog\.py list … --full")
HINT_LINK = re.compile(r"ℹ️ подсказка «backlog-list-full» показана \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC; "
                        r"полный текст — добавь --full")


def _strip_noise(text: str) -> str:
    """Стирает ЖИВЫЕ МЕТКИ ЗАМЕРА (часы, длительности) и ИЗВЕСТНЫЙ по имени вид
    подсказки «backlog-list-full» — оба варианта (полный текст И короткую ссылку),
    иначе они читались бы как разные строки при одинаковом смысле (см. разбор выше)."""
    text = NOISE.sub("~", text)
    text = HINT_FULL.sub("~ПОДСКАЗКА-backlog-list-full~", text)
    text = HINT_LINK.sub("~ПОДСКАЗКА-backlog-list-full~", text)
    return text
...
def main() -> int:
    ...
    container = d / ".mezosync"
    scripts = container / "scripts"
    shutil.copytree(LIVE_SCRIPTS, scripts)
    db = container / "mezosync.db"
    ...
    # ПРОВЕРЕНО (14.09, карточка #620, «Дополнительно»): класс беды карточки #623
    # (bite-duplicate-note.py ДО неё — чужое ЖИВОЕ объявление, приехавшее вместе со
    # снимком базы, красило приёмку под MEZO_LEASE_TEST=1) здесь НЕ ПРИМЕНИМ. Там
    # чужое объявление ЖИЛО в снимке и красило ЕДИНСТВЕННЫЙ пишущий вызов. Здесь
    # строка ниже удаляет ВСЕ tool_leases из КОПИИ сразу после снимка и ДО первого
    # случая — чужому просто неоткуда взяться к моменту, когда MEZO_LEASE_TEST
    # включает суд по этой копии (сама приёмка потом заводит ОДНО СВОЁ объявление
    # в случае ⑥ и снимает его тем же ходом). Ветка «не сужу» (код 4) там нужна была
    # ПОТОМУ, что чужое объявление не стирали; здесь оно стёрто раньше, чем могло бы
    # понадобиться, — сильнее детектора: гарантия чистого старта, а не обнаружение
    # грязного. Проверено и ПРЯМЫМ опытом: подложенное в копию чужое объявление на
    # ДРУГОМ инструменте (backlog.py) печатает предупреждение ОДИНАКОВО в обоих
    # вызовах (оно не «показывается один раз», в отличие от mezo_hints) — то есть
    # даже если бы шаг ниже почему-то не сработал, оно не создало бы НОВОГО
    # расхождения между прямым вызовом и вызовом через дверь, а не то самое, что
    # красило ① и ⑧ (см. HINT_FULL/HINT_LINK выше).
    import sqlite3
    ...
    for name, args in probes:
        ...
        equal = _strip_noise(o1) == _strip_noise(o2) and c1 == c2
        same &= equal
    ...
    # «Знак в знак» — ПОСЛЕ стирания того же ПОИМЁННОГО известного вида (см. разбор у
    # NOISE/_strip_noise выше): «list --role CORE» может так же попасть на первый/не первый
    # показ подсказки «backlog-list-full» — и это опять подсказка, не сам запуск.
    ...
    ok &= case("⑧ контроль: без объявлений вывод через него совпадает знак в знак",
               _strip_noise(o1) == _strip_noise(o2) and c1 == c2 == 0,
               "без этого случая совпадения выше могли бы держаться на случайности — "
               "сравнение ПОСЛЕ стирания известной по имени подсказки «backlog-list-full» "
               "(полный текст ⇔ короткая ссылка), не вслепую")
