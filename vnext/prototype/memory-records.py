...
def предмет_блока(тело: str) -> str:
    ...
    низ = тело.lower()
    очки = {имя: sum(1 for п in приметы if п.lower() in низ)
            for имя, приметы in ПРЕДМЕТЫ.items()}
    лучший = max(очки, key=lambda k: очки[k])
    return лучший if очки[лучший] else "разное"
...
def пересобрать(conn, роль, раздел, actor):
    ...
    conn.execute("BEGIN")
    conn.execute("DELETE FROM phoenix_records WHERE role=? AND section=?",
                 (роль, раздел))
    уцелело = 0
    ...
    print(f"  ✅ ручные поля уцелели у {уцелело} записей (тело совпало дословно)")
    if осиротели:
...
def показать(conn, роль):
    строки = conn.execute(
        ...
        "substr(replace(body, char(10), ' '), 1, 52) "
    ...
    print(f"{'id':>4} {'раздел':9} {'предмет':11} {'знаков':>7} {'час':11} "
          f"{'жив':4} начало")
    print("-" * 100)
    for id_, разд, п, зн, ч, ист, усл, жив, начало in строки:
        знак = "✅" if жив == "active" else "⚰️"
        print(f"{id_:>4} {разд:9} {п:11} {зн:>7} {ч:11} {знак:4} {начало}")
    print("-" * 100)
