...
def main() -> int:
    print("УКУС ①: умолчание штампа происхождения")
    print(f"копия базы ..... {SANDBOX}")
    prepare()

    border = max_id_now()          # граница «старого» — ДО первого своего действия
    before = stamps_upto(border)
    print(f"граница старого . #{border} (всё, что ниже, укус не создавал)")
    print(f"штампы ДО укуса  {before}\n")
    ...
    if not history_ok or not branch_ok:
        ...
        return 2
    if not fixed:
        print("\nИТОГ: ⏳ правки ещё нет. Укус исправен: он ВИДИТ дефект"
