...
ЛИНИЯ_С_ТЕКСТОМ = re.compile(r"^\s*[─═━]{3,}\s*(.+?)\s*[─═━]{3,}\s*$")
ОГРАДА = re.compile(r"^\s*```")
...
def _черт_снаружи(строки) -> int:
    ...
    for с in строки:
        ...
        if ОГРАДА.match(s):
            в_коде = not в_коде
            continue
        if not в_коде and (ЛИНИЯ.match(s) or ЛИНИЯ_С_ТЕКСТОМ.match(s)):
            n += 1
    return n
...
def _по_линиям(строки):
    ...
    for с in строки:
        s = с.rstrip("\n")
        if ОГРАДА.match(s):
            в_коде = not в_коде          # внутри ограды ни одна черта не режет
        ...
        m = None if в_коде or ОГРАДА.match(s) or ЛИНИЯ.match(s) else ЛИНИЯ_С_ТЕКСТОМ.match(s)
        if m:                            # черта с текстом — заголовок: блок НАЧИНАЕТСЯ с неё
            if текущий["строки"]:
                куски.append(текущий)
            текущий = {"тема": m.group(1)[:70], "уровень": 1, "строки": [с]}
            continue
        текущий["строки"].append(с)
        if not в_коде and ЛИНИЯ.match(s) and текущий["строки"]:
            куски.append(текущий)        # чистая черта — блок ЗАКРЫВАЕТСЯ ею, как и прежде
            текущий = {"тема": "⟨после черты⟩", "уровень": 1, "строки": []}
    if текущий["строки"]:
        куски.append(текущий)
    for к in куски:                       # тема — первая содержательная строка куска
        if not к["тема"].startswith("⟨"):  # черта с текстом уже дала тему — не затирать
            continue
        for с in к["строки"]:
            строка = с.strip()
...
def _по_абзацам(строки):
    ...
    """
    абзацы, текущий, в_коде = [], [], False
    for с in строки:
        текущий.append(с)
        if ОГРАДА.match(с.rstrip("\n")):
            в_коде = not в_коде
            continue
        if not с.strip() and not в_коде:
    ...
    for к in куски:
        ...
        тема = next((с.strip()[:70] for с in к if с.strip() and not ОГРАДА.match(с.rstrip("\n"))),
                    "⟨без первой строки⟩")
...
def блоки(текст: str) -> tuple[list[dict], str]:
    ...
    elif _черт_снаружи(строки) >= 2:
        куски, способ = _по_линиям(строки), "по чертам-разделителям (в т.ч. с текстом внутри; внутри «```» не режет)"
    else:
...
def совет(тело: str, сегодня=None) -> tuple[str, str, bool]:
    ...
    if по_возрасту and молодой:
        return "🔒", (f"слова зовут в архив ({', '.join(по_возрасту[:2])}), НО свежей дате "
                      f"{суток} сут — правило держит разборы и надгробия до {ПОРОГ_СУТОК}"), False
    ...
    if по_возрасту and суток is None:
        ...
        return "⚖️", (f"слова зовут в архив ({', '.join(по_возрасту[:2])}), но ВОЗРАСТ "
                      ...
                      f"старше {ПОРОГ_СУТОК} сут ни через неделю, ни через год. "
                      f"Хочешь, чтобы механизм звал его в архив, — ПРОСТАВЬ ДАТУ в заголовке"), True
    if г and not а:
        return "🔥", f"горячее: {', '.join(г[:3])}", False
    if а and not г:
        ...
        return "📦", f"похоже на архивное: {', '.join(а[:3])} · {возраст}", False
    if г and а:
    ...
    return "⚪", ("примет нет — смотри глазами" if суток is None
                 else f"примет нет · свежей дате {суток} сут"), False
...
def показать(conn, роль, раздел):
    ...
    print("=" * 84)
    без_даты, без_даты_знаков = 0, 0
    for i, к in enumerate(куски, 1):
        з, почему, держится = совет(к["тело"])
        if держится:
            без_даты += 1
            без_даты_знаков += к["знаков"]
    ...
    if без_даты:
        print(f"🔴 БЛОКОВ БЕЗ ДАТЫ: {без_даты} · суммарно {без_даты_знаков} знаков — "
              f"ОНИ НЕ РАЗГРУЗЯТСЯ САМИ НИКОГДА.")
        print(f"   Их приговор не изменится со временем: без даты в тексте блок не станет "
              f"старше {ПОРОГ_СУТОК} сут. Проставь дату в заголовке — и механизм")
