...
АРХИВНЫЕ_ПРИМЕТЫ = ("закрыт", "завершён", "завершен", "сделано", "⚰️", "надгробие",
                    "отозван", "снято", "история", "прежн", "было:", "оплачено")
...
def совет(тело: str) -> tuple[str, str]:
    ...
    if а and not г:
        return "📦", f"похоже на архивное: {', '.join(а[:3])}"
    if г and а:
        return "⚖️", f"и то и другое ({', '.join(г[:2])} / {', '.join(а[:2])}) — решай сам"
    return "⚪", "примет нет — смотри глазами"
