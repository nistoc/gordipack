...
Случаи (различающий = механизм обязан промолчать или ответить ИНАЧЕ):
  ...
  ④ своя записка СТАРШЕ памяти — предупреждения НЕТ         РАЗЛИЧАЮЩИЙ
...
def main() -> int:
    ...
    ok &= case("③ своя записка новее памяти — блок требует читать её первой",
               "ОНА НОВЕЕ СОХРАНЁННОЙ ПАМЯТИ" not in text,
               "своя записка (08:00) СТАРШЕ памяти (09:00) ⇒ предупреждения быть НЕ должно",
               differ=True)
    ...
    ok &= case("④ своя записка НОВЕЕ памяти — предупреждение появляется",
               "ОНА НОВЕЕ СОХРАНЁННОЙ ПАМЯТИ" in text2 and "#105" in text2,
               "тот же блок на сдвинутых данных отвечает ИНАЧЕ — значит различает, "
               "а не печатает всегда одно", differ=True)

    ok &= case("⑤ правила после сохранения памяти названы поимённо, старое НЕ попало",
               "свежее-правило v3" in text and "старое-правило" not in text,
               "подложено два правила, одно правлено ПОСЛЕ сохранения памяти → назваться обязано одно",
               differ=True)
    ...
    ok &= case("⑥ отметки чтения нет — сказано вслух, а не пропущено молча",
               "отметки прочитанного у роли НЕТ" in text3,
