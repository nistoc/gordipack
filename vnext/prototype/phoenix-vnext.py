...
def derive(con, role, notes=6):
    ...
    a(f"- отметка прочитанного: {cur[0] if cur else '—'} (обновлена {cur[1] if cur else '—'} UTC), голова ленты #{head}")
    ...
    if not ob:
        ...
        a(f"- след оборванного чтения: батч до #{ob[1]} (выдан {ob[2]} UTC) так и не подтверждён, "
          f"но отметка прочитанного {cur[0]} уже дальше — ноты не потеряны, читались заново")
    else:
