...
═══ ЧТО ПРОИЗОШЛО СО МНОЙ 2026-08-06 18:26 UTC ═══
    ...
    ack по этому токену                   → курсор 3069 → 3109
...
def main() -> int:
    print("УКУС: перевыданный батч подтверждает больше, чем показал")
    print(f"копия базы ..... {SANDBOX}")
    print(f"курсор старт ... #{START_CURSOR}\n")
    prepare()
    ...
    print("\nСВОЙСТВО: курсор после подтверждения не должен уйти выше ПОКАЗАННОГО")
    print(f"   показано до .......... #{top_shown}")
    print(f"   курсор встал на ...... #{after}")
    ...
    print(f"   показано до #{max(ids_a)} · курсор #{after_a} · "
          f"{'✅ совпадает' if same_ok else '❌ расходится'}")
    print("   (если бы и здесь расходилось — укус ловил бы не перевыдачу, а что-то другое)")
    ...
    # и с этой минуты укус меняет роль: он СТОРОЖ ПРОТИВ ВОЗВРАТА.
    ...
    if not (ok and same_ok):
        ...
        return 1
    print("\nИТОГ: ✅ СВОЙСТВО ДЕРЖИТСЯ — курсор не уходит выше показанного,")
    print("      различающий случай прошёл (значит укус мерит перевыдачу, а не что-то ещё)")
    return 0
