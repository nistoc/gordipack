...
def main() -> int:
    ...
    try:
        ...
        # ⑩ НАШ СОБСТВЕННЫЙ ВОПРОС К СОСЕДУ НЕ ВМЕНЯЕТСЯ НАМ КАК ЕГО.
        # 🪤 Найдено 22.08 на живых файлах. Второе слово имени значит РАЗНОЕ у двух
        # соглашений: у старого («ask.кто-тема.md», общая папка, писали обе стороны) это
        # АВТОР, у нового («ask.кому.тема.md», своя исходящая) — АДРЕСАТ. Проверка знала
        # одно правило и применяла его ко всем именам: два наших письма к соседу числились
        # его вопросами к нам и должны были покраснеть через 48 ч как «мы не ответили» —
        # на свои же письма. Погасить это нельзя ничем: ответить себе, удалить своё письмо
        # или ждать — всё мимо, а роль научится не верить красному.
        d10 = tmp / "i"
        guard10 = build(d10, with_ask=False, with_answer=False, with_box=False)
        ...
        ok &= case("⑩ НАШ вопрос к соседу (новое имя) не вменяется нам как его вопрос",
                   ...
                   "и погасить это нельзя ни одним своим действием", differ=True)

        # ⑪ ВСТРЕЧНЫЙ К ⑩, И ОН ВАЖНЕЕ САМОГО ⑩: различает ли проверка ВИД ИМЕНИ или просто
        # разучилась смотреть в нашу папку. Оба файла лежат в ОДНОЙ папке — виден обязан
        # быть ровно один.
        d11 = tmp / "j"
        guard11 = build(d11, with_ask=False, with_answer=False, with_box=False)
        ...
        ok &= case("⑪ ВСТРЕЧНЫЙ: в ТОЙ ЖЕ папке его вопрос старого вида — виден",
                   ...
                   "в нашу папку: тогда чинили бы не различение, а слепоту", differ=True)

        # ⑫ ОТВЕТ ДРУГОМУ СОСЕДУ НЕ ГАСИТ ВОПРОС ЭТОГО.
        # 🪤 Прежняя редакция собирала имена ответов ПЛОСКО по всем папкам моста и теряла,
        # кому ответ адресован. Сегодня в двух мостах разом живут темы про одно и то же —
        # совпадение перестало быть умозрительным. Цена ошибки: вопрос числится отвеченным,
        ...
        ok &= case("⑫ ответ ДРУГОМУ соседу не засчитывается ответом этому",
                   "ask.atlas.thing.md" in out and "отвечен" not in out,
                   "тема та же, адресат другой: засчитать — значит объявить сделанным то, "
                   "чего сосед не получал", differ=True)

        # ⑬ 🩸 РЕГРЕССИЯ, НАЙДЕННАЯ СОСЕДЯМИ 23.08 16:58 UTC — и просьба назвать свойство
        # ЯВНО, а не покрыть случайно. Их замер: контур БЕЗ каталога согласования, мост
        # видит ВОПРОСЫ и не видит ОТВЕТЫ ⇒ пять ложных красных на сделанной работе.
        # ⛔ Это хуже, чем «мост не проверяется вовсе»: пробел заметен, а уверенное ложное
        # красное на сделанном учит НЕ ВЕРИТЬ КРАСНОМУ.
        # 🎯 Их класс, забираю дословно: ПРИЁМКА ЛОВИТ ТО, ЧТО ЕЙ ПОКАЗАЛИ, И МОЛЧИТ
        # О ТОМ, ЧЕГО В НЕЙ НЕТ. Двенадцать случаев выше уже идут без каталога
        # согласования — но НИ ОДИН этого не утверждал, и свойство держалось случайно.
        # ⚖️ Причина была в том, что мост отсчитывался от каталога согласования; теперь он
