# -*- coding: utf-8 -*-
r"""ПРИЁМКА накопления --reviewed в write-message.py — карточка #253.

🩸 ЧЕМ ОПЛАЧЕНО (COORD, 25.08 08:07 UTC): три «--reviewed <файл>» подряд молча записали
ОДИН жест — последний; строка «разбор записан» об одном имени прочлась подтверждением
всей операции. Молчащий отказ в подвиде «частичный успех»: треть работы сделана,
отчёт о ней честен, об остальных двух третях — тишина.

Случаи (различающий = обязан ответить ИНАЧЕ, а не одинаково):
  ① повтор флага ×3 ......... ТРИ жеста в bridge_reviewed, три строки в выводе  РАЗЛИЧАЮЩИЙ
  ② список через запятую .... те же ТРИ — числа форм РАВНЫ (критерий карточки)  КОНТРОЛЬ
  ③ смешанная форма ......... «--reviewed a,b --reviewed c» → три               РАЗЛИЧАЮЩИЙ
  ④ ОБРАТНЫЙ ХОД: накопление снято (прежнее объявление) → случай ① даёт ОДИН
    у сломанной копии — разница и есть доказательство                           РАЗЛИЧАЮЩИЙ

⛔ Живой базы не пишет: каждый прогон — своя копия.
"""
from __future__ import annotations

import os
import pathlib
import shutil
import sqlite3
import subprocess
import sys
import tempfile

sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
import mezo_paths  # noqa: E402

SCRIPTS = mezo_paths.container_root(__file__) / ".mezosync" / "scripts"
WRITER = SCRIPTS / "write-message.py"
LIVE_DB = mezo_paths.live_db()
CASES = DIFFER = 0


def case(title, verdict, detail, differ=False):
    global CASES, DIFFER
    CASES += 1
    DIFFER += bool(differ)
    print(f"{'✅' if verdict else '🔴'} {title}")
    print(f"   {detail}")
    return verdict


def gesture_count(db, label):
    con = sqlite3.connect(f"file:{pathlib.Path(db).as_posix()}?mode=ro", uri=True)
    n = con.execute("SELECT COUNT(*) FROM bridge_reviewed WHERE file_name LIKE ?",
                    (f"{label}%",)).fetchone()[0]
    con.close()
    return n


def run_tool(db, note_path, label, *forms, script=WRITER):
    # Тело ноты — СВОЁ на случай: писатель отвергает повтор тела как дубль (код 3),
    # и случаи ②③④ на общей ноте умирали об эту защиту, а не о предмете приёмки.
    note_path.write_text(f"проба приёмки жестов разбора {label} — записка стенда\n",
                    encoding="utf-8")
    # PYTHONPATH на каталог писателя: слабая копия живёт в чужом каталоге, и без него
    # умирает на ПЕРВОМ ЖЕ импорте (mezo_paths, dryrun…) — красный был бы гибелью
    # копии, а не работой накопления. Копировать модули поимённо нельзя: список протухает.
    env = dict(os.environ, MEZO_CONTAINER=str(mezo_paths.container_root(__file__)),
               PYTHONPATH=str(SCRIPTS))
    r = subprocess.run([sys.executable, str(script), "--db", str(db), "--role", "PROTO",
                        "--file", str(note_path), *forms],
                       capture_output=True, text=True, encoding="utf-8",
                       errors="replace", timeout=300, env=env)
    return r.returncode, (r.stdout or "") + (r.stderr or ""), gesture_count(db, label)


def main() -> int:
    ok = True
    d = pathlib.Path(tempfile.mkdtemp(prefix="bite-reviewed-"))
    try:
        db = d / "sand.db"
        shutil.copy(LIVE_DB, db)
        note_path = d / "нота.md"

        # ① повтор флага ×3.
        code, output, n1 = run_tool(db, note_path, "п1-",
                                "--reviewed", "п1-один.md",
                                "--reviewed", "п1-два.md",
                                "--reviewed", "п1-три.md")
        ok &= case("① повтор флага ×3 → ТРИ жеста, три строки «разбор записан»",
                   code == 0 and n1 == 3 and output.count("разбор записан") == 3,
                   f"код {code}; жестов {n1}; ровно здесь COORD получил 1 из 3 молча",
                   differ=True)

        # ② список через запятую — числа форм равны (критерий).
        code, output, n2 = run_tool(db, note_path, "п2-",
                                "--reviewed", "п2-один.md,п2-два.md,п2-три.md")
        ok &= case("② список через запятую → те же ТРИ (формы равны — критерий карточки)",
                   code == 0 and n2 == 3 and n2 == n1,
                   f"код {code}; жестов {n2} против {n1} у повтора — числа сошлись",
                   differ=True)

        # ③ смешанная форма.
        code, output, n3 = run_tool(db, note_path, "п3-",
                                "--reviewed", "п3-один.md,п3-два.md",
                                "--reviewed", "п3-три.md")
        ok &= case("③ смешанная форма → три",
                   code == 0 and n3 == 3,
                   f"код {code}; жестов {n3}; смешение форм — самый вероятный живой вызов",
                   differ=True)

        # ④ ОБРАТНЫЙ ХОД: накопление снято → случай ① даёт ОДИН у сломанной.
        # 🩸 НАЙДЕНО (задача PROTO, замер 2026-09-13): здесь стояли ДВА str.replace() подряд
        # без проверки, что образец нашёлся — если бы ВТОРОЙ образец (кусок/piece) не
        # совпал, первый всё равно менял бы текст, "break_patch == original_text" молчал бы
        # (тексты РАЗНЫЕ), и приёмка тихо гоняла бы НЕДОпорченную копию, читая её как полный
        # обратный ход. Лечится КАК У СОСЕДНИХ приёмок (см. weaken()/ослабить() в
        # bite-interview-gates.py и bite-claim-second-hand.py): каждый образец порчи обязан
        # найтись РОВНО ОДИН раз — иначе громкий отказ, а не молчаливое продолжение.
        original_text = WRITER.read_text(encoding="utf-8")
        PATTERN_ARG = ('parser.add_argument("--reviewed", action="append",'
                       ' default=None, metavar="ФАЙЛ",')
        PATTERN_LOOP = ("for name in [n.strip() for piece in args.reviewed\n"
                        "                         for n in piece.split(\",\") if n.strip()]:")
        for pattern in (PATTERN_ARG, PATTERN_LOOP):
            if original_text.count(pattern) != 1:
                raise SystemExit(f"⛔ ПРИЁМКА НЕ СОСТОЯЛАСЬ: образец порчи не найден в "
                                 f"{WRITER.name}: «{pattern}»")
        break_patch = original_text.replace(PATTERN_ARG,
                              'parser.add_argument("--reviewed", default=None,'
                              ' metavar="ФАЙЛ",', 1)
        break_patch = break_patch.replace(
            PATTERN_LOOP,
            "for name in [n.strip() for n in args.reviewed.split(\",\") if n.strip()]:", 1)
        weak_path = d / "прежний.py"
        weak_path.write_text(break_patch, encoding="utf-8")
        code4, _, n4 = run_tool(db, note_path, "п4-",
                             "--reviewed", "п4-один.md",
                             "--reviewed", "п4-два.md",
                             "--reviewed", "п4-три.md", script=weak_path)
        ok &= case("④ ОБРАТНЫЙ ХОД: накопление снято — у сломанной жестов ОДИН",
                   n4 == 1 and n1 == 3,
                   f"слабая {n4} против настоящей {n1} — теряет именно СНЯТОЕ накопление"
                   f" (код слабой {code4})", differ=True)
    finally:
        shutil.rmtree(d, ignore_errors=True)

    print()
    print(f"{'✅ НАКОПЛЕНИЕ ЖЕСТОВ ПРИНЯТО' if ok else '🔴 НЕ ПРИНЯТО'} — случаев {CASES},"
          f" различающих {DIFFER}")
    return 0 if ok else 1


if __name__ == "__main__":
    sys.exit(main())
