...
только читается):
  ① активный пул есть (ЕДИНСТВЕННЫЙ), карточка заводится БЕЗ --track → рождается
    ЗАМОРОЖЕННОЙ с условием разморозки (карточка #399 ступень ③ поглотила прежнее
    предупреждение: при одном активном наборе оно заменено заморозкой)
...
case("① без --track при ЕДИНСТВЕННОМ активном пуле → карточка ЗАМОРОЖЕННАЯ (#399)",
     rc == 0 and "ЗАМОРОЖЕННОЙ" in out and "frozen" in out)
