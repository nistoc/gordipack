...
"""bite-tool-lease — приёмка объявления о правке инструмента (карточка #204).
    ...
    python <КОНТУР>/vnext-tools/bite-tool-lease.py
...
def main() -> int:
    ...
    o2, c2 = run([str(scripts / "backlog.py"), "--db", str(db), "list", "--role", "PROTO"],
                 role="CORE")
    # ⚠️ Ожидание у двух членов группы РАЗНОЕ — и это не дыра, а карточка #391:
    # `backlog.py list` — ЧИТАЮЩАЯ подкоманда (список в самом инструменте), под чужим
    # объявлением она предупреждает и работает. Что группа НАКРЫВАЕТ backlog.py,
    # доказывает само предупреждение; пишущий член группы отказывает по-прежнему.
    # Границу подкоманд целиком судит bite-lease-read-subcommands.py.
    ok &= case("⑧ одно объявление закрывает связанную группу из нескольких имён",
               c1 == 3 and c2 == 0 and "В РАБОТЕ" in o2,
               f"пишущий член группы отказал ({c1}), читающая подкоманда предупреждена "
               f"и работает ({c2}) — объявлять связанные порознь значило бы дать зовущему "
               f"половину правды", differ=True)
