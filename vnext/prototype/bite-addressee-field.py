...
def main() -> int:
    ...
    # ═══ Карточка #453 (TAXO при приёмке #440): предпосылкой объясняется ТОЛЬКО
    # исход с rc==0. При упавшей ЗАПИСИ предпосылка цела, вывода нет по другой
    # причине — прежний текст посылал чинить подсадку, а сломана была запись.
    if code7 != 0:
        суть7 = (f"запись НЕ ПРОШЛА (rc={code7}, ждём 0) — вывода нет, "
                 f"предупреждение и номер судить нечем")
    else:
        суть7 = (f"запись rc=0 · предупреждение о спящем («ч назад» выше подтверждения): "
                 f"{'НАПЕЧАТАНО' if предупр7 else 'ОТСУТСТВУЕТ — предпосылка опыта (старый след спящего) не состоялась'}")
    ok &= case("⑦ номер записки взят из ПОДТВЕРЖДЕНИЯ — часы сна адресата его не подменяют",
               ...
               суть7 + f" · разобран номер {mid7} · в базе {got7}", differ=True)
