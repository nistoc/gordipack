...
def main() -> int:
    ...
    ok &= case("⑦ номер записки взят из ПОДТВЕРЖДЕНИЯ — часы сна адресата его не подменяют",
               code7 == 0 and "ч назад" in out7 and got7 == [("ZZSONYA", "to", "field")],
               f"предупреждение о спящем (число часов ВЫШЕ подтверждения) в выводе есть, "
               f"разобран номер {mid7} · в базе {got7} — утренние «в базе []» были бы "
               f"ровно здесь", differ=True)
