...
def main():
    ...
    if before_fingerprint != fingerprint(db):
        # ⚡ КАРТОЧКА #629: по умолчанию (без явного --db) приёмка с 07-25 судит СВОЮ
        # согласованную копию живой базы (mezo_stand.snapshot_db) — писать в неё, кроме
        # прогонов этого же набора, некому. Строка обязана называть правду про КАЖДУЮ
        # из двух баз, а не только про явно названную.
        if a.db:
            print("       ⚪ файл базы за это время менялся — ЧУЖОЙ рукой; на вердикт не влияет")
        else:
            print("       ⚪ файл копии базы менялся — это собственные прогоны набора (их служебные "
                  "следы), а не чужая рука; что из этого попало в память, поимённо судит случай ⑪")
