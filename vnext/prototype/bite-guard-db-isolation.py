...
# Ирония, которую стоит сохранить: приёмка, написанная ПРОТИВ впечатанного пути к базе
# (карточка #168), сама держала два таких пути — и лежала в ПУБЛИЧНОМ каталоге, где
# путь машины ещё и утечка. Правило, применённое к предмету и не применённое к себе,
# держится ровно до первого, кто посмотрит.
GUARD = mezo_target.script("guard-write-without-read.py")
...
def make_clean(db: Path) -> None:
    """Довести КОПИЮ до заведомо чистого состояния: все курсоры к голове ленты.

    🪤 ЗАВЕДЕНО 2026-08-10 18:42 UTC, КОГДА ЭТА ПРИЁМКА ПОКРАСНЕЛА НА СЕБЕ САМОЙ.
    Случай ① звался «контроль: чистая копия», но чистой копию никто не делал — её просто
    копировали с живой базы и НАДЕЯЛИСЬ, что живая чиста. Вчера так и было, и приёмка
    была зелёной. Сегодня @ING честно поработал — написал записку позже, чем читал ленту, —
    и «контроль» покраснел, ничего не сломавшись.
    🎯 ЭТО ТОТ ЖЕ КЛАСС, РАДИ КОТОРОГО ПРИЁМКА И НАПИСАНА (карточка #168): вердикт зависел
    от постороннего состояния. Я вычистил его из ИСПЫТУЕМОГО механизма и оставил в САМОЙ
    приёмке — а её зелёный читался как доказательство изоляции.
    ⚖️ Мораль сильнее случая: «копия живого» и «известное состояние» — разные вещи.
       Контрольная половина пары обязана быть ПОСТРОЕНА, иначе она проверяет погоду.

    ⚠️ И ВТОРАЯ ПОЛОВИНА УРОКА, СТОИВШАЯ ЕЩЁ ОДНОГО ПРОГОНА: сперва я двигал только курсор
    к голове — и копия ОСТАЛАСЬ КРАСНОЙ. Признак судит не по числу непрочитанных, а по
    ВРЕМЕНАМ: «непрочитанных 0, писал в 18:27, читал в 16:24». Я привёл в порядок то,
    что счёл предметом проверки, вместо того что она измеряет, — и узнал об этом прогоном,
    а не чтением. ⇒ Чистота ставится по ОБОИМ полям, которые смотрит признак.
    """
    con = sqlite3.connect(db)
    head = con.execute("SELECT COALESCE(MAX(id), 0) FROM messages").fetchone()[0]
    con.execute("UPDATE read_cursors SET last_read_id = ?, updated_at = datetime('now')",
                (head,))
    con.commit()
    con.close()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # ⚰️ И ТРЕТИЙ ВПЕЧАТАННЫЙ ПУТЬ БЫЛ ЗДЕСЬ — его показал греп, а не чтение шапки
        # (снято 2026-08-10 18:49 UTC). Два убрал глазами, третий бы уехал: два и три
        # различает только прогон по СПИСКУ ФОРМ.
        agg = mezo_target.script("guard-all.py")
