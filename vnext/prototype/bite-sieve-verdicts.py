...
каталоге. Решето берётся СОСЕДНЕЕ (эта пара каталогов сверяется проверкой дрейфа).
"""
...
FIXTURE: list[tuple[str, str, bool]] = [
    ...
    # ── НОВЫЕ СЛОВА: ТО ЖЕ САМОЕ, СКАЗАННОЕ ПОСЛЕ ПЕРЕВОДА ВЫВОДА (18.08) ──────
    # ⚠️ Различающие. Инструменты с 17.08 печатают «отметка версии» и «проверок»
    # вместо прежних слов; памяти, написанные завтра, будут говорить так же.
    # Признак, знающий одно написание, промолчит на них — и молчание прочтут
    # как «память чиста». Ровно этим переименование могло ослепить проверки.
    ("state",    "схема на отметке версии v4 — держим её как есть", True),
    ("state",    "в контуре 24 проверки, все зелёные", True),
]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        case("⑤ проверка памяти на устаревшие утверждения по-прежнему называет свою границу",
    ...
    print("   спрятанный В КАВЫЧКАХ, правило цитаты замолчит — это названо в самой проверке "
          "памяти на устаревшие утверждения,")
