...
def find(candidates, what):
    # 🪤 ВЫБОР ИЗ НЕСКОЛЬКИХ УБРАН — слово владельца 2026-08-07 13:26 UTC: «убери этот выбор».
    #    Прежде бралась ПЕРВАЯ существующая, и если существовали две, вторая молча
    #    проигрывала. Цена такого «возьму подходящее» замерена в тот же день: в соседнем
    #    скрипте подстановка «какой-нибудь» колонки дала 1808 записок за сутки при 1806 всего,
    #    и скрипт при этом НЕ УПАЛ. Молчаливый выбор выглядит как работа.
    #    ⇒ Ровно одна — берём. Несколько — падаем и требуем назвать явно.
    found = [p for p in candidates if p.exists()]
    if len(found) == 1:
        return found[0]
    if len(found) > 1:
        print(f"⛔ НЕ ЗАВЕДЁН: {what} найден в НЕСКОЛЬКИХ местах, и выбирать за тебя я не буду:")
        for p in found:
            print(f"   · {p}")
        print("   ⇒ укажи нужный явным флагом. Тихий выбор одного из двух — это ошибка,")
        print("     которая выглядит как успех, и находится она потом и дорого.")
        sys.exit(2)
