...
DEADLINE_WORD = re.compile(
    "стоп|срок|остановк|прекра|не ранее|не раньше|не позже|дедлайн|"
    "сверки прекращ|окончани|пауз|возврат|стартует|проснусь|пробужден", re.I)
# ⚡ 05.09 03:15 UTC: словарь пополнен словами ПАУЗЫ. Слово владельца 04.09 22:57 «ЧЕРЕЗ 4 ЧАСА.
# Не раньше» девять ролей записали как «не раньше 02:57 UTC» / «возврат 03:07 UTC» / «проснусь
# 03:11 UTC» — и после наступления часа записка #4745 (PROTO) покраснела, #4746 (STUD) и #4754
# (CHROME) пожелтели. Ровно класс карточки #516, второй живой случай: у роли своё слово срока.
# Словарь по-прежнему НЕ гасит — понижает до жёлтого при совпавшем смещении и слове РЯДОМ.
            ...
            if ((tagged or mention.is_mention(line, kinds=("correction",)))
                ...
                for r in refs:
                    ...
                    if n >= mid:                    # поправка ПОСЛЕ ошибки, не до
                        continue
                    # 🎯 КАРТОЧКА #520 (TAXO, 05.09): строка ЕСТЬ поправка и тема совпала —
                    # но ИМЕННО ЭТОТ номер в ней может стоять примером или носителем самой
                    # поправки. Тогда гашение убирает не дефект, а РАЗГОВОР о нём — вместе
                    # с дефектом. Замер по живой ленте: таких 4 из 19 действующих гашений.
                    if mention.fix_target(line, n)[0] == "ссылка":
                        continue
...
def selftest():
    ...
    def стенд_база(имя):
        ...
        # 🩸 ВИД ЛЕНТЫ — ОПЛАЧЕНО ВТОРЫМ ПОПАДАНИЕМ В ТУ ЖЕ ДВЕРЬ (05.09, находка STUD).
        # 16:41 UTC живая база получила вид messages_all (архив ленты по возрасту), разбор
        # выше перевели на него — а стенд остался со старой схемой. Прогон падал «нет такой
        # таблицы», и ВОСЕМЬ случаев из девятнадцати не выполнялись с того часа: самые тонкие,
        # про гашение поправок и про срок против дефекта.
        # ⚡ И первый раз тоже записан здесь же, комментарием выше: тогда живая схема получила
        # КОЛОНКУ, сегодня — ВИД. Урок стоял там, где его читает правящий, и не сработал:
        # он называл ЧАСТНЫЙ случай («стенды несут колонку tags»), а не правило «стенд обязан
        # догонять живую схему». Роль сверяет названную колонку, находит её на месте и идёт дальше.
        # 👉 Настоящее лечение назвал STUD и оно НЕ ЗДЕСЬ: стенд самопроверки строить не руками,
        # а тем же шагом схемы, что и живую базу — тогда «стенд не догнал» перестаёт быть
        # возможным состоянием. Это заявка, а пока — строка ниже.
        c.execute("CREATE VIEW messages_all AS SELECT id, writer_role, timestamp, "
