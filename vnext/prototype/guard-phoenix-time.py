    ...
    for i, line in enumerate((body or "").splitlines(), 1):
        for m in STAMP.finditer(line):
            ...
            if t > сейчас:
                ...
                out.append((f"🟢 НАЗВАННЫЙ СРОК, не метка события: час лежит в БУДУЩЕМ "
                            f"от прогона ({сейчас:%Y-%m-%d %H:%M} UTC) — не судится",
                            f":{i}", line.strip()[:100]))
                continue
            delta = t - saved
            for off in OFFSETS:
                if abs(delta - off) <= TOLERANCE:
                    ...
                    elif DEADLINE_WORD.search(line[max(0, m.start() - ОКНО_СЛОВ):
                                                   m.end() + ОКНО_СЛОВ]):
