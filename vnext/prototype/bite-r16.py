...
def with_deps(reader, tmp):
    """Ридер импортирует mezo_paths — кладём его рядом с временной копией ридера,
    иначе опыт упадёт на импорте и это будет выглядеть как провал свойства."""
    dest = tmp / reader.name
    shutil.copy2(reader, dest)
    dep = reader.parent / "mezo_paths.py"
    if dep.exists():
        shutil.copy2(dep, tmp / "mezo_paths.py")
    return dest
...
def verify(target, sandbox):
    ...
    if unread < 1:
        die(f"в песочнице нечего читать роли {ROLE} (непрочитанных {unread})",
            "пересними песочницу: python vnext/sandbox/bootstrap.py")
    ...
    named = bool(re.search(r"за ним ещё (\d+) нот|УПЁРСЯ В ЛИМИТ", b9))
    ok9 = (got >= int(advised)) or named
    print(f"P9 совет инструмента исполним: советовал --limit {advised} → выдал {got} нот · "
          f"остаток назван: {'да' if named else 'НЕТ'} "
          f"→ {'✅' if ok9 else '🔴 роль выполнила совет и получила МОЛЧАЛИВЫЙ короткий ответ'}")
    verdicts.append(ok9)
