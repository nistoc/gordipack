...
Случаи:
  ...
  ⑧-бис контроль ПОРЧИ (карточка #455): активные наборы живой базы не погашены —
     первый ход подсадки правит чужие строки, и вставочных следов не оставляет;
     судится состоянием (активных 0 → красное со словом «погашено: N»), не разницей
"""
import os
import shutil
import sqlite3
...
# к её ПЕРВОМУ ходу — массовой ПОРЧЕ чужих строк (гашение всех активных наборов).
# Судится СОСТОЯНИЕМ живой базы, не разницей с началом прогона: разницу даёт
# и чужая запись соседа (беда карточки #444, от которой уходили).
# ⚖️ ГРАНИЦА вслух: мир, где контур ЗАКОННО живёт без активного набора, суд
# не отличит от порчи — красное там ложное, но оно ГОВОРИТ ПОИМЁННО, и приёмщик
# разберётся; тихая порча наборов девяти ролей дороже одной ложной тревоги.
con = sqlite3.connect(f"file:{LIVE_DB.as_posix()}?mode=ro", uri=True)
активных = con.execute("SELECT COUNT(*) FROM tracks WHERE status='active'").fetchone()[0]
пауз = con.execute("SELECT COUNT(*) FROM tracks WHERE status='paused'").fetchone()[0]
con.close()
case("⑧-бис ПОРЧА чужих строк: активные наборы живой базы НЕ погашены (карточка #455)",
     активных > 0,
     f"наборов задач погашено (paused): {пауз} · активных 0 — ход подсадки "
     f"«погасить активные» мог исполниться в ЖИВОЙ базе" if активных == 0
     else f"активных наборов: {активных} — первый ход подсадки живой базы не коснулся")
