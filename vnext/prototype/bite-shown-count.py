"""
bite-shown-count.py — укус на правку ② : журнал выдач помнит, СКОЛЬКО ПОКАЗАЛ, и не врёт об этом.
...
починил ПРИЧИНУ (дефект перевыдачи, укус bite-r16-reissue.py) — и с этой минуты расхождения
...
def prepare() -> None:
    """Копия живой базы. Живая НЕ ТРОГАЕТСЯ — у перископа и укусов один закон."""
...
def main() -> int:
    print("УКУС ②: журнал выдач помнит, сколько показал, и не врёт об этом")
