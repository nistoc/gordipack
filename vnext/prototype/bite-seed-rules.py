...
def стенд(правила, метка=ДОГОН, посев_битый=False, дубль=None):
    ...
    тело_файла = ("-- посев\n"
                  ...
                  + ",\n".join(строки) + ";\n")
    if дубль:
        ключ_д, тело_д, объявлен = дубль
        if объявлен:
            тело_файла += (f"-- ⚠️ ВЫШЕ МЁРТВОЕ ОПРЕДЕЛЕНИЕ: тот же ключ '{ключ_д}' "
                           f"определён ещё раз ниже, в базу доедет оно\n")
        тело_файла += ("INSERT OR REPLACE INTO rules (rule_key, body, locked_by, version) "
                       f"VALUES\n('{ключ_д}',\n '{тело_д}',\n 'owner', 1);\n")
    if посев_битый:
...
def main() -> int:
    ...
    КОРОТКО = ПРАВИЛО[1].split(".")[0] + "."
    посев8, db8 = стенд([(*ПРАВИЛО, "2026-08-01 10:00")],
                        дубль=("общее-правило", КОРОТКО, False))
    out8, code8 = run(посев8, db8)
    ...
    посев8б, db8б = стенд([(*ПРАВИЛО, "2026-08-01 10:00")],
                          дубль=("общее-правило", КОРОТКО, True))
    out8б, code8б = run(посев8б, db8б)
    ok &= case("⑧-бис ВСТРЕЧНЫЙ: дубль ОБЪЯВЛЕН маркером → назван без красноты",
               code8б == 0 and "ОБЪЯВЛЕН" in out8б and "БЕЗ объявления" not in out8б,
    ...
    d8т = mezo_stand.new("bite-seed-dup-")
    db8т = d8т / "live.db"
    con8 = sqlite3.connect(str(db8т))
    ...
    con8.commit()
    con8.close()
    посев8т = d8т / "seed.sql"
    посев8т.write_text(
        "-- посев\n-- ПОСЛЕДНИЙ ДОГОН ПОСЕВА: " + ДОГОН + "\n"
        "INSERT OR REPLACE INTO rules (rule_key, body, locked_by, version) VALUES\n"
        "('к430',\n 'РАННЕЕ ТЕЛО.',\n 'owner', 1);\n"
        ...
        "('к430',\n 'ПОЗДНЕЕ ТЕЛО.',\n 'owner', 1);\n",
        encoding="utf-8")
    out8т, code8т = run(посев8т, db8т)
    ok &= case("⑧-тер вердикт совпадает с РАЗВЁРНУТОЙ базой: судится доезжающее",
               code8т == 0 and "совпадают дословно 1" in out8т,
