...
# Замер 29.08 09:14 UTC: строка «⚠️ Зови так: <относительная форма>» — ПРЕДПИСАНИЕ,
# а голые ⚠️/⛔ стояли в приметах отзыва и гасили её как надгробие. Ложное МОЛЧАНИЕ
# (слепота): роли пишут наказы именно так — значок внимания перед инструкцией.
# Различитель: СЛОВО отмены гасит всегда (как прежде); ГОЛЫЙ значок гасит только
# строку БЕЗ предписывающего слова — «⛔ <форма>» остаётся честным надгробием,
# «⚠️ Зови так: …» судится. Выкинуть значки целиком запрещено телом карточки:
# ложные красные на честных надгробиях дороже пропуска.
WORD_MARK = re.compile(r"(НЕЛЬЗЯ|нельзя|не зови|вместо|F20|R15a"
                       r"|прежн|раньше|было:)", re.I)
SIGN_MARK = re.compile(r"[⛔⚠]")
# «не зови» ловится словом ВЫШЕ и сюда не доходит; просмотр назад отсекает «…не зови»
# в незнакомой обёртке, чтобы отрицание не сошло за предписание.
PRESCRIBE = re.compile(r"(?<!не )(зови|запускай|выполняй|используй|вызывай)", re.I)
...
class _Revoked:
    ...
    def search(self, text):
        ...
        if m:
            return m
        # Голый значок гасит ТОЛЬКО строку без предписания (карточка #361):
        # «⚠️ Зови так: …» — инструкция, и она обязана судиться.
        if SIGN_MARK.search(t) and not PRESCRIBE.search(t):
            return SIGN_MARK.search(t)
        return None
